...
function career(){
 ...
 <section class="card"><h2>版本</h2><div class="log"><strong>V1.9.6.8</strong>｜派對與遠端社交修正版：手機聊天邀約跨區可接受、成人派對自行選人並逐一顯示接受／拒絕、伴侶分手風險、懷孕與生父不確定事件。</div></section>`;
}
function bind(){
...
function partyAcceptanceChance(c,adult){const p=state.player,rel=p.relations[c.name]||0,traits=safeTraits(c).join("、");let ch=.22+rel*.006;if(/外向|冒險|自由/.test(traits))ch+=.10;if(/內向|保守|責任感強|忠誠/.test(traits))ch-=adult?.12:.04;if(adult&&(c.teammatePartnerOf||c.relationshipType==="女友"||c.relationshipType==="老婆"))ch-=.16;return clamp(ch,.06,.92)}
...
function maybePartyPregnancy(guests){const p=state.player;p.adultLife=p.adultLife||{};p.adultLife.pregnancies=p.adultLife.pregnancies||[];const women=guests.map(n=>state.characters?.[n]).filter(c=>c?.gender==="女"&&Number(c.age||0)>=18);const events=[];women.forEach(c=>{if(Math.random()>=.055)return;const already=p.adultLife.pregnancies.some(pg=>pg.name===c.name&&!pg.born&&pg.status!=="已結束");if(already)return;const uncertain=guests.length>=3&&Math.random()<.38;const pg={name:c.name,week:state.date.week,year:state.date.year,progressWeeks:0,status:"可能懷孕",birthPending:false,source:"成人私人派對",fatherKnown:!uncertain,father:uncertain?null:p.name,possibleFathers:uncertain?[p.name,"其他成年派對參與者"]:[p.name]};p.adultLife.pregnancies.push(pg);events.push(uncertain?`${c.name} 出現懷孕可能；由於派對情境，目前無法確定生父`:`${c.name} 出現懷孕可能`);state.logs.push(`🤰 ${events[events.length-1]}。`)});return events}
function finishPrivateParty(adult,acceptedNames){const p=state.player;if(!acceptedNames.length){modal(`<h2>🎉 派對取消</h2><p>這次受邀者都沒有接受，因此沒有舉辦派對，也不消耗生活時段。</p>${closeBtn()}`);return}if(!consume(adult?"成人私人派對":"私人派對",1))return;const guests=acceptedNames.map(n=>state.characters?.[n]).filter(Boolean);guests.forEach(c=>p.relations[c.name]=clamp((p.relations[c.name]||0)+rand(1,3),0,100));const breakups=adult?guests.map(partyPartnerFallout).filter(Boolean):[];const pregnancies=adult?maybePartyPregnancy(acceptedNames):[];const exposure=adult&&Math.random()<clamp(.006+guests.length*.0035+(guests.some(c=>c.publicFigure)?.012:0),.006,.055);if(exposure){changeCareerRep(-rand(6,14),"私人成人派對曝光");p.prCrisis={type:"私人成人派對照片外流",severity:rand(3,5),source:"匿名外流"};state.news.unshift("🚨 夜鋒高度私密的成人派對內容外流，引發贊助商與媒體關注。")}state.logs.push(`🎉 ${adult?"成人私人":"私人"}派對完成，來賓 ${acceptedNames.join("、")}${exposure?"；事件意外曝光":"；維持私人"}。`);save();render();modal(`<h2>🎉 派對結束</h2><p>實際參加者：${acceptedNames.join("、")}</p>${breakups.length?`<div class="notice badtext">💔 ${breakups.join("；")}。</div>`:""}${pregnancies.length?`<div class="notice">🤰 ${pregnancies.join("；")}。後續會進入既有懷孕追蹤；生父不確定時不會直接認定夜鋒是父親。</div>`:""}${exposure?`<div class="notice badtext">⚠️ 低機率曝光事件發生，造成較大的公關風險。</div>`:`<div class="notice goodtext">活動維持私人，沒有影響職業風評。</div>`}${closeBtn()}`)}
