...
function maybeTVProgram1984(){const w=ensureLivingWorld1984(),pc=state.player.proCareer;if(Math.random()>.09)return;const shows=["電競夜話","王者聊天室","深夜選手席","賽場之外"],show=shows[stableAgeOffset(`${state.date.year}-${state.date.week}`,shows.length)],guest=Object.keys(pc.rivals||{})[0]||((pc.roster||[]).find(x=>!x.isSub)?.name)||"聯賽新秀",host=ensureGoddessHost1983();const actualGuest=Math.random()<.35?state.player.name:guest,text=`📺《${show}》本週邀請 ${actualGuest} 談版本與職業生活，由 ${host.name} 主持。`;w.tvHistory.unshift({show,host:host.name,guest:actualGuest,year:state.date.year,week:state.date.week});w.worldFeed.unshift({type:"節目",text,year:state.date.year,week:state.date.week});if(actualGuest===state.player.name)state.messages.push({id:`tv-${Date.now()}`,from:`《${show}》製作組`,text:`我們想邀請夜鋒參加本週節目，由 ${host.name} 主持，主題會談近期版本、戰隊與宿敵。`,unread:true,resolved:false,type:"mediaEventInvite",eventTopic:`《${show}》節目錄影`})}
