...
function fallbackTraits(c){
 const male=["努力","老實","外向","冷靜","好勝","溫和","講義氣","謹慎","野心","責任感強","內向","冒險","領袖氣質","大心臟","慢熱","容易上頭","自大","懶惰","急性子","耍大牌","玻璃心","記仇","嫉妒心強","完美主義","團隊型","晚熟","固執"];
 const female=["努力","老實","可愛","天然呆","成熟","現實","溫柔","好勝","獨立","理性","浪漫","高依附","責任感強","冒險","內向","外向","冷靜","敏感","忠誠","自由奔放","自大","懶惰","急性子","耍大牌","玻璃心","記仇","嫉妒心強","完美主義","團隊型","大心臟","慢熱"];
...
function freshLocalName1983(country,gender){const parts={台灣:{女:[["林","陳","張","許","蘇","葉","周","沈"],["語晴","若希","映彤","芷寧","心妍","沛晴","婕安","予柔"]],男:[["陳","林","張","許","周","葉","沈","黃"],["奕辰","柏勳","承宇","子謙","昱翔","景皓","家睿","廷安"]]},韓國:{女:[["金","李","朴","崔","姜"],["智妍","瑞恩","秀彬","恩彩","多賢"]],男:[["金","李","朴","崔","姜"],["俊赫","敏奎","志勳","賢宇","道允"]]},日本:{女:[["佐藤","高橋","鈴木","田中"],["美月","結衣","凜","葵","陽菜"]],男:[["佐藤","高橋","鈴木","田中"],["悠真","蓮","海斗","翔","湊"]]}};const set=parts[country]?.[gender];if(set){for(let i=0;i<30;i++){const n=set[0][rand(0,set[0].length-1)]+set[1][rand(0,set[1].length-1)];if(!state.characters?.[n]?.known)return n}}const base=gender==="女"?["Mia","Emma","Nina","Lena","Sophie","Ava"]:["Leo","Noah","Ethan","Felix","Ryan","Alex"];for(let i=0;i<30;i++){const n=`${base[rand(0,base.length-1)]} ${["Chen","Lin","Lee","Park","Meyer","Wilson"][rand(0,5)]}`;if(!state.characters?.[n]?.known)return n}return `${gender==="女"?"新朋友":"新朋友"}${state.date.year}${Date.now()%10000}`}
function createResidenceEncounter(context){
 const p=state.player,{country,city}=currentLocationProfile();
 if(Math.random()<.90){
   const female=Math.random()<.52,pools=localEncounterPool(country),pool=female?pools.女:pools.男,unseen=pool.filter(n=>!state.characters?.[n]?.known),name=unseen.length?unseen[rand(0,unseen.length-1)]:freshLocalName1983(country,female?"女":"男");
...
function openMessage(id){
 ...
 const replySets={
  ...
  mediaEventInvite:[["join","參加活動"],["decline","這次婉拒"]],
  mentorStory:[["guide","願意指導他"],["observe","先看一場比賽再說"],["decline","暫時婉拒"]],
...
function resolveMessage(m,r){
 ...
 if(m.type==="mentorStory"){
  const name=m.from,rw=ensureRealism1985(),w=ensureLivingWorld1984(),st=rw.mentorStories[name]||(rw.mentorStories[name]={stage:0,trust:0});
  if(r==="decline"){st.stage=-1;st.declinedYear=state.date.year;w.pendingMentor=null;m.resolved=true;m.replied=true;m.replyText="目前先專注自己的賽季。";remember1984(name,"師徒契機",`夜鋒暫時婉拒了 ${name} 的長期指導請求。`,1);}
  else {st.stage=Math.max(1,st.stage);st.trust+=(r==="guide"?15:8);st.playerAccepted=true;st.nextStoryWeek=state.date.week+2;m.resolved=true;m.replied=true;m.replyText=r==="guide"?"有問題可以來找我，我會認真看。":"我先看你的比賽，再告訴你我看到什麼。";remember1984(name,"師徒契機",`${name} 的請教得到夜鋒回應，正式進入觀察與指導階段。`,1);}
  save();document.querySelector('.modal-backdrop')?.remove();render();return;
 }
 if(m.type==="mediaEventInvite"){
  if(r==="join"){
    if(remain()<1){modal(`<h2>行程已滿</h2><p>參加電競圈活動需要1格生活時段。</p>${closeBtn()}`);return}
    if(!consume("電競圈活動",1))return;
    const host=m.from,topics=["版本改動與職業選手適應","選手與媒體的幕後交流","聯賽明星選手圓桌","戰隊文化與職涯故事"],topic=m.eventTopic||topics[stableAgeOffset(host+state.date.week,topics.length)];
    const newPeople=["陳語珊","許庭瑜","葉子謙","蘇婕寧","高承宇","沈若凡"].filter(n=>n!==host&&!state.characters?.[n]?.known);let met="";
    if(newPeople.length&&Math.random()<.75){const n=newPeople[0];addSocialAcquaintance(n,rand(10,20),{gender:socialStableHash(n,"g")%2?"女":"男",age:22+stableAgeOffset(n,9),career:"電競媒體人",occupation:"電競媒體人",identityType:"媒體",acquaintanceSource:"電競圈活動",traits:fallbackTraits({name:n,gender:"女"})});met=`活動結束前，你也透過 ${host} 認識了 ${n}。`;}
    state.player.followers+=rand(80,260);state.player.relations[host]=clamp((state.player.relations[host]||0)+2,0,100);state.logs.push(`🎟️ 參加 ${host} 邀請的電競圈活動：${topic}。${met}`);
    m.resolved=true;m.replied=true;m.replyText="我會參加。";save();document.querySelector('.modal-backdrop')?.remove();render();
    modal(`<h2>🎟️ 電競圈活動</h2><p>你接受 ${host} 的邀請並到場參加。</p><div class="notice"><strong>本次主題：${topic}</strong><br>現場有選手、主持人、記者與業界人士交流；你也接受了簡短訪談並和其他來賓交換意見。</div>${met?`<div class="notice goodtext">🤝 ${met}</div>`:""}<p class="small">粉絲小幅增加，與 ${host} 的關係提升。後續可能延伸成節目邀約、採訪或新的媒體人脈。</p>${closeBtn()}`);return;
  }else{m.resolved=true;m.replied=true;m.replyText="這次先不參加。";state.logs.push(`📱 婉拒 ${m.from} 的電競圈活動邀請。`);save();document.querySelector('.modal-backdrop')?.remove();render();return}
 }
...
migrateProV1937();migrateProV1938();
}
...
function ensureLegacyChildSystem(){
 ...
 arr.forEach(pg=>{
  if(!pg.born)return;
  pg.child=pg.child||{name:`${pg.name.slice(0,1)}小星`,age:0,gender:(pg.childGender||(pg.childGender=Math.random()<.5?"男":"女")),public:false,birthYear:state.date.year,bond:0};
...
function finishBirthEvent(pg,choice){
 const p=state.player;pg.birthPending=false;pg.born=true;pg.status="孩子已出生";pg.child=pg.child||{name:`${pg.name.slice(0,1)}小星`,age:0,gender:(pg.childGender||(pg.childGender=Math.random()<.5?"男":"女")),public:false,birthYear:state.date.year,bond:0};
...
function tacticMetaFit(key){const m=state.world?.meta||{},txt=JSON.stringify(m),rh=ensureLivingWorld1984().rhythm?.key||"balanced";let v=0;if(key==="botcarry"&&/ADC|下路|射手/.test(txt))v+=5;if(key==="midjungle"&&/中路|打野|節奏/.test(txt))v+=4;if(key==="teamfight"&&/團戰|龍/.test(txt))v+=4;if(key==="scaling"&&/後期|發育/.test(txt))v+=4;if(key==="junglecarry"&&/打野|野區/.test(txt))v+=4;const map={early:["tempo","midjungle","engage"],jungle:["junglecarry","objective","midjungle"],bot:["botcarry","doublecarry","scaling"],top:["topside","split","sidelane"],teamfight:["teamfight","engage","disengage"],macro:["objective","rotation","scaling"],split:["split","oneThreeOne","sidelane"]};if((map[rh]||[]).includes(key))v+=7;return v}
function tacticAbilityCeiling(key){const t=TACTIC_DEFS[key]||TACTIC_DEFS.midjungle,ratings=t.roles.map(rosterRoleRating),heroes=t.roles.map(roleHeroReadiness),core=Math.min(...ratings),roleAvg=ratings.reduce((a,b)=>a+b,0)/ratings.length,heroAvg=heroes.reduce((a,b)=>a+b,0)/heroes.length;/* V1.9.9.4: a weaker role should matter, but must not single-handedly crush an otherwise elite lineup. */return clamp(Math.round(roleAvg*.72+heroAvg*.18+core*.10+4),45,98)}
function tacticLineupSnapshot1994(){const p=state.player,pc=p.proCareer||{},roles=["上路","打野","中路","ADC","輔助"],out={};for(const role of roles){if(role===(p.role==="下路"?"ADC":p.role)){out[role]=p.name;continue}const m=(pc.roster||[]).filter(x=>!x.isSub&&(x.role===role||(role==="ADC"&&x.role==="下路"))).sort((a,b)=>rosterRating(b)-rosterRating(a))[0];out[role]=m?.name||null}return out}
function enforceTacticCeilings1994(reason="陣容檢查"){const pc=state.player.proCareer;if(!pc?.tactics)return;Object.keys(TACTIC_DEFS).forEach(k=>{const cap=tacticAbilityCeiling(k),cur=Number(pc.tactics.mastery[k]||45);if(cur>cap){pc.tactics.mastery[k]=cap;state.logs.push(`🧠 ${TACTIC_DEFS[k].name}：因${reason}，熟練度調整為 ${cap}（不得高於目前陣容上限）。`)}})}
function syncTacticLineup1994(reason="陣容異動"){const pc=state.player.proCareer;if(!pc?.tactics)return;const now=tacticLineupSnapshot1994(),prev=pc.tactics.lineupSnapshot1994;if(!prev){pc.tactics.lineupSnapshot1994=now;enforceTacticCeilings1994(reason);return}const changed=Object.keys(now).filter(r=>prev[r]!==now[r]);if(changed.length){Object.entries(TACTIC_DEFS).forEach(([k,t])=>{const hit=t.roles.filter(r=>changed.includes(r)).length;if(!hit)return;const cur=Number(pc.tactics.mastery[k]||45),loss=Math.min(16,4+hit*3+Math.max(0,cur-80)*.08);pc.tactics.mastery[k]=Math.max(35,cur-loss)});pc.tactics.lineupSnapshot1994=now;state.logs.push(`🔄 ${reason}：${changed.join("、")}人員更換，相關戰術默契下降；需透過Scrim與正式比賽重新磨合。`)}enforceTacticCeilings1994(reason)}
...
function suggestTactic(k){const pc=ensureCareer20(),coach=ensureCoachProfile(),trust=pc.coachTrust||50,chance=clamp(.18+trust*.004+(coach?.skills?.management||65)*.002, .2,.82),accepted=Math.random()<chance;pc.tactics.playerSuggestion={tactic:k,accepted,week:state.date.week};if(accepted){pc.tactics.trainingFocus=k;pc.tactics.selected=k;state.logs.push(`💬 你向教練提出「${TACTIC_DEFS[k].name}」方向，教練願意納入本週Scrim戰術準備。`)}else state.logs.push(`💬 你提出「${TACTIC_DEFS[k].name}」建議，但教練決定維持自己的戰術判斷。`);save();render();modal(`<h2>🧑‍🏫 戰術溝通</h2><p>${accepted?`教練接受你的意見，會把「${TACTIC_DEFS[k].name}」納入準備；正式比賽仍由教練最終決定。`:`教練聽取意見後沒有採納，正式比賽仍依教練團判斷。`}</p>${closeBtn()}`)}
...
function formerTeamEquityRefund(team,reason="轉會前股份回購"){
 const p=state.player,pc=ensureCareer20(),e=pc.clubEquity||{},pct=Number(e.shares?.[team]||0);if(!team||pct<=0)return 0;
 e.costBasis=e.costBasis||{};const basis=Number(e.costBasis[team]||0),estimate=Math.round(equityPricePerPct()*pct),refund=Math.max(0,basis||estimate);
 p.cash+=refund;e.shares[team]=0;e.costBasis[team]=0;e.refundHistory=e.refundHistory||[];e.refundHistory.unshift({team,pct,refund,year:state.date.year,week:state.date.week,reason});
 state.logs.push(`🏢 ${team} 在夜鋒離隊前回購其 ${pct.toFixed(1)}% 股份，返還 NT$${refund.toLocaleString()}。股份結清後才可完成轉會。`);return refund;
}
function hasMajorTransferConsultClause(){const c=state.player.proCareer?.contract;return !!(c?.demands||[]).some(x=>String(x).includes("重大轉會先溝通"))}
function pendingMajorTransferCard(){const pc=state.player.proCareer,o=pc?.pendingMajorTransfer;if(!isProfessionalStage()||!o)return "";const pct=Number(pc.clubEquity?.shares?.[o.from]||o.sharePct||0),salary=Number(pc.contract?.salary||0);return `<section class="card"><h2>📨 重大轉會事前溝通</h2><div class="notice badtext"><strong>${o.from} → ${o.buyer}</strong><br>這只是正式報價，尚未成交。依「重大轉會先溝通」條款，管理層必須先與夜鋒協商。<br>目前月薪：NT$${salary.toLocaleString()}${pct>0?`<br>你仍持有 ${pct.toFixed(1)}% ${o.from} 股份；股份未經你同意出售並完成結算前，禁止完成轉會。`:""}</div><div class="reply-grid">${pct>0?`<button class="reply" onclick="sellEquity1989('all',true)">協商出售全部股份</button>`:""}<button class="reply" onclick="negotiateSalary1989()">與原戰隊協商薪資</button><button class="reply" onclick="resolveMajorTransferConsultation1989('proceed')">願意聽取轉會方案</button><button class="reply" onclick="resolveMajorTransferConsultation1989('object')">拒絕目前方案</button></div></section>`}
function negotiateSalary1989(){const pc=state.player.proCareer,o=pc?.pendingMajorTransfer,c=pc?.contract;if(!o||!c)return;const cut=Math.max(20000,Math.round(c.salary*.9/1000)*1000);modal(`<h2>🤝 留隊薪資協商</h2><p>${o.from} 可以因薪資壓力與你協商，但不能單方面降薪。</p><div class="notice">現月薪 NT$${c.salary.toLocaleString()}｜管理層試探方案 NT$${cut.toLocaleString()}</div><div class="reply-grid"><button id="acceptSalaryTalk1989" class="reply">接受降至 NT$${cut.toLocaleString()}</button><button id="keepSalary1989" class="reply">維持原合約</button></div>${closeBtn()}`);document.querySelector('#acceptSalaryTalk1989')?.addEventListener('click',()=>{c.salary=cut;o.salaryNegotiated=true;o.keepPlayerPreferred=true;state.logs.push(`🤝 夜鋒與 ${o.from} 協商後自願將月薪調整為 NT$${cut.toLocaleString()}，作為留隊方案的一部分。`);document.querySelector('.modal-backdrop')?.remove();save();render()});document.querySelector('#keepSalary1989')?.addEventListener('click',()=>{o.salaryNegotiated=false;state.logs.push(`📄 夜鋒選擇維持與 ${o.from} 的原月薪合約；戰隊不得單方面降薪。`);document.querySelector('.modal-backdrop')?.remove();save();render()})}
function sellEquity1989(mode='all',fromTransfer=false){const p=state.player,pc=ensureCareer20(),team=fromTransfer?(pc.pendingMajorTransfer?.from||pc.team):pc.team,e=pc.clubEquity||{},held=Number(e.shares?.[team]||0);if(held<=0)return modal(`<h2>🏢 股份出售</h2><p>目前沒有可出售的 ${team} 股份。</p>${closeBtn()}`);const pct=mode==='all'?held:Math.min(held,Number(mode)||0),basis=Number(e.costBasis?.[team]||0),perPct=team===pc.team?equityPricePerPct():Math.round((basis&&held?basis/held:equityPricePerPct())||0),price=Math.max(0,Math.round(perPct*pct));modal(`<h2>🏢 股權出售協商</h2><p>出售 ${team} ${pct.toFixed(1)}% 股份給原戰隊／既有股東。</p><div class="notice">協議回購價：NT$${price.toLocaleString()}<br>成交後持股 ${(held-pct).toFixed(1)}%</div><div class="reply-grid"><button id="confirmSellEquity1989" class="primary">確認出售</button></div>${closeBtn()}`);document.querySelector('#confirmSellEquity1989')?.addEventListener('click',()=>{p.cash+=price;e.shares[team]=Math.max(0,held-pct);e.costBasis=e.costBasis||{};e.costBasis[team]=Math.max(0,basis-(held?basis*(pct/held):0));e.saleHistory=e.saleHistory||[];e.saleHistory.unshift({team,pct,price,year:state.date.year,week:state.date.week});const o=pc.pendingMajorTransfer;if(o&&o.from===team)o.sharePct=e.shares[team];state.logs.push(`🏢 夜鋒同意出售 ${team} ${pct.toFixed(1)}% 股份，收到 NT$${price.toLocaleString()}。`);document.querySelector('.modal-backdrop')?.remove();save();render()})}
function resolveMajorTransferConsultation1989(choice){const pc=state.player.proCareer,o=pc?.pendingMajorTransfer;if(!o)return;const tm=ensureTransferMarket(),x=(tm.listings||[]).find(v=>v.id===o.listingId);if(!x){pc.pendingMajorTransfer=null;save();render();return}if(choice==='object'){x.status='未售出';x.pendingConsultation=false;x.buyer=null;x.buyerRegion=null;x.destination=null;pc.pendingMajorTransfer=null;state.logs.push(`🗣️ 夜鋒拒絕目前的重大轉會方案，交易未成立，繼續留在 ${o.from}。`);save();render();return}const pct=Number(pc.clubEquity?.shares?.[o.from]||0);if(pct>0){state.logs.push(`⛔ 重大轉會暫停：夜鋒仍持有 ${o.from} ${pct.toFixed(1)}% 股份，必須先完成自願股權出售協議。`);save();render();return modal(`<h2>⚠️ 尚不能完成轉會</h2><p>你仍持有 ${pct.toFixed(1)}% ${o.from} 股份。請先協商出售股份；系統不會自動沒收或強制回購。</p>${closeBtn()}`)}x.status='已成交';x.buyer=o.buyer;x.buyerRegion=o.buyerRegion;x.destination=o.toAcademy?'次級聯賽／二隊':'一軍';x.soldWeek=state.date.week;x.pendingConsultation=false;pc.pendingMajorTransfer=null;applyPlayerTransferListing(x,o.buyer,o.buyerRegion,o.toAcademy,true);tm.history.unshift({...x});state.news.unshift(`🔄 轉會成交：${x.name}｜${x.team} → ${o.buyer}${o.toAcademy?' 次級聯賽／二隊':''}。`);save();render()}
function resolveMajorTransferConsultation1988(choice){return resolveMajorTransferConsultation1989(choice)}
...
function apprenticeAction(){const p=state.player,pc=ensureCareer20(),l=pc.legacy,w=ensureLivingWorld1984(),qualified=(pc.careerStats?.matches||0)>=60&&starPowerForPlayer()>=55;if(!qualified)return modal(`<h2>🎓 師徒</h2><p>師徒之路需要累積足夠的職業比賽經驗與圈內地位。不是固定年齡解鎖；當條件成熟，新秀會開始主動找上門。</p>${closeBtn()}`);const db=ensureGlobalProDatabase(),pool=Object.values(db).flatMap(r=>Object.values(r).flat()).filter(x=>x.active&&!x.retired&&x.age<=21&&x.name!==p.name&&!l.apprentices[x.name]);if(!pool.length)return modal(`<h2>🎓 師徒</h2><p>目前沒有合適的新生代選手。真正值得長期指導的新人不會每年大量出現。</p>${closeBtn()}`);const x=(w.pendingMentor&&pool.find(v=>v.name===w.pendingMentor))||pool.sort((a,b)=>(b.potential||b.rating)-(a.potential||a.rating))[0];l.apprentices[x.name]={name:x.name,role:x.role,team:x.team,progress:10,sinceYear:state.date.year};w.pendingMentor=null;addSocialAcquaintance(x.name,45,{isPro:true,role:x.role,team:x.team,age:x.age,identityType:"職業選手・徒弟",acquaintanceSource:"主動請教夜鋒"});remember1984(x.name,"師徒",`${x.name} 正式開始固定向夜鋒請教。`,2);state.logs.push(`🎓 ${x.name} 正式開始向夜鋒請教，建立師徒關係。`);save();render();modal(`<h2>🎓 師徒之路開啟</h2><p>${x.name} 正式成為夜鋒指導的新秀。未來他的成長、轉隊、交手與重大賽事都可能產生師徒事件。</p>${closeBtn()}`)}
...
function ensureSecondLife1993(){const pc=ensureCareer20();pc.secondLife=pc.secondLife||{retired:false,considering:false,retirement:null,roles:[],coachJob:null,schoolCoachJob:null,guestLectures:[],broadcastLevel:0,streamLevel:0,managementLevel:0,history:[]};return pc.secondLife}
function retirementCard1993(){if(!isProfessionalStage())return"";const sl=ensureSecondLife1993();if(sl.retired)return secondLifeCard1993();return `<section class="card"><h2>🌅 退役與第二人生</h2><div class="small">退役沒有最低年齡。連敗、傷病、家庭、失去熱情或已完成目標，都可以成為退役理由；不會一按就立刻結束。</div><button id="considerRetirement1993" class="reply">考慮退役</button></section>`}
function openRetirement1993(){const pc=ensureCareer20(),sl=ensureSecondLife1993(),reasons=["長期連敗與競技疲勞","傷病／健康考量","家庭與生活","已完成職業目標","失去職業熱情","想轉往教練／經營／媒體"];modal(`<h2>🌅 考慮退役</h2><p>先選擇目前最主要的想法。這只是啟動退役討論，不會立刻退役。</p><div class="reply-grid">${reasons.map(x=>`<button class="reply retirement-reason1993" data-reason="${x}">${x}</button>`).join("")}</div>${closeBtn()}`);document.querySelectorAll('.retirement-reason1993').forEach(b=>b.onclick=()=>retirementTalk1993(b.dataset.reason))}
function retirementTalk1993(reason){const pc=ensureCareer20(),sl=ensureSecondLife1993();sl.considering=true;sl.reason=reason;sl.history.unshift({year:state.date.year,week:state.date.week,type:"萌生退意",reason});save();modal(`<h2>🗣️ 退役討論</h2><div class="notice">主要原因：${reason}</div><p>你可以立即退役、宣布打完本賽季後退役，或暫時不做決定。戰隊、家人與重要人物之後會對這項決定產生反應。</p><div class="reply-grid"><button id="retireNow1993" class="reply">立即正式退役</button><button id="retireSeason1993" class="reply">打完本賽季後退役</button><button id="retirePause1993" class="reply">暫緩決定</button></div>${closeBtn()}`);document.querySelector('#retireNow1993').onclick=()=>finalizeRetirement1993("立即退役");document.querySelector('#retireSeason1993').onclick=()=>{sl.plan="賽季結束後退役";state.logs.push(`🌅 夜鋒公開內部決定：將在本賽季結束後退役。`);save();render()};document.querySelector('#retirePause1993').onclick=()=>{sl.considering=false;sl.plan=null;save();render()}}
function finalizeRetirement1993(mode){const p=state.player,pc=ensureCareer20(),sl=ensureSecondLife1993();if(sl.retired)return;sl.retired=true;sl.considering=false;sl.retirement={year:state.date.year,week:state.date.week,age:p.age,team:pc.team,salary:pc.contract?.salary||0,reason:sl.reason||mode,lastMatch:pc.lastMatch?JSON.parse(JSON.stringify(pc.lastMatch)):null,ability:Math.round(avg()),followers:p.followers,worldTitles:Number(pc.worldTitles||0)};sl.roles=[...(sl.roles||[]),"退役職業選手"];pc.legacy.postCareer=pc.legacy.postCareer||{path:"第二人生",progress:0};state.news.unshift(`🌅 ${p.name} 宣布退役｜${sl.retirement.age}歲｜最後效力 ${pc.team}｜${sl.retirement.reason}`);state.logs.push(`🌅 夜鋒正式結束職業選手生涯，第二人生開啟。`);save();render();modal(`<h2>🌅 職業選手篇落幕</h2><p>夜鋒正式宣布退役。股份、家庭、NPC關係、財產、師徒、宿敵與歷史全部保留；接下來可發展教練、經營、媒體、實況與母校路線。</p>${closeBtn()}`)}
function secondLifeCard1993(){const sl=ensureSecondLife1993();if(!sl.retired)return"";const coach=sl.coachJob?`${sl.coachJob.team}・${sl.coachJob.role}`:"無正式教練職";return `<section class="card"><h2>🌇 第二人生</h2><div class="notice">正式教練職：${coach}<br>其他身分：${(sl.roles||[]).join("、")||"退役選手"}</div><div class="small">正式教練職只能一個；職業隊教練與母校電競社正式教練互斥。職業教練仍可短期受邀返校擔任客座講師。</div><div class="reply-grid"><button class="reply secondlife-role1993" data-role="職業教練">尋找職業教練工作</button><button class="reply secondlife-role1993" data-role="母校教練">回母校正式執教</button><button class="reply secondlife-role1993" data-role="媒體">發展主播／賽評</button><button class="reply secondlife-role1993" data-role="經營">投入俱樂部經營</button><button class="reply secondlife-role1993" data-role="實況">經營實況</button>${sl.coachJob?.kind==="職業"?`<button class="reply secondlife-role1993" data-role="客座講師">受邀返校客座講課</button>`:""}</div></section>`}
function chooseSecondLifeRole1993(role){const pc=ensureCareer20(),sl=ensureSecondLife1993();if(role==="職業教練"){if(sl.schoolCoachJob)return modal(`<h2>⚠️ 身分衝突</h2><p>你目前是母校電競社正式教練，必須先結束該職務才能擔任職業隊正式教練。</p>${closeBtn()}`);sl.coachJob={kind:"職業",team:pc.team,role:"助理教練",sinceYear:state.date.year};sl.roles=[...new Set([...(sl.roles||[]),"職業教練"])]}else if(role==="母校教練"){if(sl.coachJob)return modal(`<h2>⚠️ 身分衝突</h2><p>正式職業隊教練不能同時長期執教母校。可改以客座講師形式返校。</p>${closeBtn()}`);sl.schoolCoachJob={kind:"母校",team:"夜鋒高中電競社",role:"正式教練",sinceYear:state.date.year};sl.coachJob={kind:"母校",team:"夜鋒高中電競社",role:"正式教練",sinceYear:state.date.year};sl.roles=[...new Set([...(sl.roles||[]),"母校電競社教練"])]}else if(role==="客座講師"){sl.guestLectures.push({year:state.date.year,week:state.date.week,school:"夜鋒高中",topic:"職業賽場與戰術理解"});state.logs.push("🏫 夜鋒以職業教練身分返母校進行短期客座講座；不構成第二份正式教練職。")}else{const map={"媒體":"特約賽評","經營":"俱樂部經營者","實況":"實況主"};sl.roles=[...new Set([...(sl.roles||[]),map[role]||role])]};save();render()}
function msiReturnStory1993(){if(!isProfessionalStage())return;const p=state.player,pc=ensureCareer20(),sl=ensureSecondLife1993();if(proAnnualPhase()!=="MSI"||!playerQualifiedForInternational("MSI"))return;pc.msiHistory=pc.msiHistory||[];if(pc.msiHistory.some(x=>x.year===state.date.year))return;const prev=pc.msiHistory.slice().sort((a,b)=>b.year-a.year)[0],gap=prev?state.date.year-prev.year:null;pc.msiHistory.push({year:state.date.year,team:pc.team});if(gap&&gap>=3){const worlds=(p.achievements||[]).filter(x=>/世界賽冠軍/.test(x.title||x.name||"")).length+Number(!!pc.worldChampionYear);const nick=gap>=5?"五月失蹤人口回歸":"季中舞台久違的老面孔";state.news.unshift(`🌍 ${gap}年等待結束——${worlds?`${worlds}屆世界冠軍 `:""}夜鋒重返MSI舞台。`);ensureLivingWorld1984().worldFeed.unshift({type:"MSI",text:`💬 社群熱議：「${nick}」——夜鋒相隔 ${gap} 年再次取得MSI資格。`,year:state.date.year,week:state.date.week})}}
...
function ensureGoddessHost1983(){const pc=ensureCareer20();pc.mediaNetwork=pc.mediaNetwork||[];let x=pc.mediaNetwork.find(v=>v.specialId==="host_goddess_1983"||v.name==="韓知恩");if(!x){x={name:"韓知恩",job:"新生代電競主持人",relation:28,specialId:"host_goddess_1983",starPower:82};pc.mediaNetwork.push(x)}addSocialAcquaintance("韓知恩",28,{gender:"女",age:23,birthYear:state.date.year-23,identityType:"新生代電競主持人",career:"電競主持人",occupation:"電競主持人",acquaintanceSource:"職業賽事媒體圈",romanceable:true,publicFigure:true,important:true,traits:["外向","機靈","專業","好勝"],desc:"快速竄紅的新生代電競主持人，擅長臨場追問與選手故事訪談。"});return x}
function mediaNetworkCard(){if(!isProfessionalStage())return "";const pc=ensureCareer20(),major=majorMediaContext();ensureGoddessHost1983();return `<section class="card"><h2>🎙️ 媒體與電競圈</h2><div class="small">大型專訪會由不同主持人／記者輪替；媒體人物也會有自己的職涯與人際關係。</div>${pc.mediaNetwork.length?`<div class="notice">媒體人脈：${pc.mediaNetwork.slice(0,8).map(x=>`${x.name}（${x.job}）`).join("、")}</div>`:""}<div class="notice">✨ 新生代主持焦點：韓知恩｜高人氣電競主持人｜會與既有主持人輪流負責重要專訪。</div>${major?`<div class="notice">📡 當前焦點：${major.title}<br>符合大型專訪條件，會在相關賽事／事件流程中觸發完整問答。</div>`:`<div class="small">目前沒有需要大型專訪的焦點事件。</div>`}<button id="opponentInterview1983" class="reply">🎤 查看對手賽前採訪</button></section>`}
function ensureMediaHost(){const pc=ensureCareer20(),jobs=["電競主播","賽事主持人","電競記者","賽後採訪主持","賽評"],names=["周子晴","林以辰","夏妍","陳洛","許芷晴","高宇衡"];ensureGoddessHost1983();let regular=pc.mediaNetwork.find(v=>v.specialId!=="host_goddess_1983");if(!regular){const name=names[stableAgeOffset(pc.team+state.date.year,names.length)],job=jobs[stableAgeOffset(name,jobs.length)];regular={name,job,relation:30};pc.mediaNetwork.push(regular);addSocialAcquaintance(name,30,{gender:/晴|妍|芷/.test(name)?"女":"男",age:22+stableAgeOffset(name,9),identityType:job,career:job,occupation:job,acquaintanceSource:"重大賽事專訪",romanceable:/晴|妍|芷/.test(name)})}const goddess=pc.mediaNetwork.find(v=>v.specialId==="host_goddess_1983");pc.mediaHostTurn=(pc.mediaHostTurn||0)+1;return pc.mediaHostTurn%2===0?goddess:regular}
function opponentPregameInterview1983(){const pc=ensureCareer20(),scheduled=currentScheduledProMatch(),opp=scheduled?.opp||currentProOpponent()?.name;if(!opp)return modal(`<h2>🎤 對手賽前採訪</h2><p>目前尚未確定下一場正式比賽對手。</p>${closeBtn()}`);const team=opp,db=ensureGlobalProDatabase();let players=[];for(const reg of PRO_REGIONS){const arr=db?.[reg]?.[team];if(Array.isArray(arr)){players=arr;break}}const star=players.slice().sort((a,b)=>(b.rating||0)-(a.rating||0))[0],name=star?.name||`${team} 隊長`,rating=star?.rating||78,host=ensureGoddessHost1983();const lines=rating>=90?["我們知道夜鋒很強，但系列賽不是一個人的遊戲。","這種舞台就是為了擊敗最強的對手而存在。"]:rating>=82?["我們準備了針對Nova的打法，臨場執行會決定結果。","尊重對手，但我們不是來陪跑的。"]:["能和強隊交手是機會，我們會把能做的全部打出來。","壓力在對方，我們只需要專注自己的比賽。"],quote=lines[stableAgeOffset(team+state.date.week,lines.length)];pc.opponentInterviewHistory=pc.opponentInterviewHistory||[];pc.opponentInterviewHistory.unshift({year:state.date.year,week:state.date.week,team,player:name,quote});modal(`<h2>🎤 對手賽前採訪</h2><p class="small">${host.name}｜${host.job}</p><div class="notice"><strong>${team}｜${name}</strong><br>「${quote}」</div><p class="small">採訪內容依目前實際賽程對手與對方代表選手產生，不會改變已鎖定的比賽對手。</p>${closeBtn()}`)}
...
function startMajorInterview(ctx,after){const pc=ensureCareer20(),host=ensureMediaHost(),scheduled=currentScheduledProMatch(),opp=scheduled?.opp||currentProOpponent()?.name||"下一個對手";if(after==="match"&&scheduled?.opp)pc.lockedMatchOpponent=scheduled.opp;const questions=contextualInterviewQuestions1984(ctx,host,opp);pc.activeInterview={ctx,host,questions,index:0,answers:[],after};showInterviewQuestion()}
...
function career(){
 ...
 <section class="card"><h2>版本</h2><div class="log"><strong>V1.9.8.7</strong>｜世代交替・人物故事深化：全NPC年齡同步、Aster超級新人、玩家參與式師徒故事，並保留個性／狀態／化學反應與活世界系統。</div></section>`;
}
function bind(){
 document.querySelector(".tactic-advice")?.addEventListener("click",openTacticAdvice);document.querySelectorAll(".tactic-suggest").forEach(b=>b.onclick=()=>suggestTactic(b.dataset.tactic));document.querySelectorAll(".equity-buy").forEach(b=>b.onclick=()=>buyEquity(+b.dataset.pct));document.querySelectorAll(".equity-direct").forEach(b=>b.onclick=()=>equityDirective(b.dataset.role));document.querySelectorAll(".career-shift").forEach(b=>b.onclick=()=>careerShift(b.dataset.path));document.querySelector(".coach-suggest")?.addEventListener("click",()=>coachChangeProposal(false));document.querySelector(".coach-change")?.addEventListener("click",()=>coachChangeProposal(true));document.querySelector("#capitalInjection")?.addEventListener("click",injectClubCapital);document.querySelector("#clubBankLoan")?.addEventListener("click",()=>clubTakeLoan1982(false));document.querySelector("#clubSeekSponsor")?.addEventListener("click",()=>clubSeekSponsor1982(false));document.querySelector("#clubLaunchMerch")?.addEventListener("click",launchClubMerch1982);document.querySelector("#openingInterview1985")?.addEventListener("click",startSeasonOpeningInterview1985);document.querySelector("#acceptClubEquityOffer")?.addEventListener("click",acceptClubEquityOffer1982);document.querySelector("#declineClubEquityOffer")?.addEventListener("click",declineClubEquityOffer1982);document.querySelector("#seekApprentice")?.addEventListener("click",apprenticeAction);document.querySelector("#opponentInterview1983")?.addEventListener("click",opponentPregameInterview1983);document.querySelector("#tvWorld1984")?.addEventListener("click",showTVWorld1984);document.querySelector("#considerRetirement1993")?.addEventListener("click",openRetirement1993);document.querySelectorAll(".secondlife-role1993").forEach(b=>b.onclick=()=>chooseSecondLifeRole1993(b.dataset.role));document.querySelector("#rivalWorld1984")?.addEventListener("click",showRivalWorld1984);
...
function proScrim(){
 ...
 syncTacticLineup1994("Scrim前陣容");const focus=pc.tactics?.trainingFocus,tk=(focus&&TACTIC_DEFS[focus]?focus:(pc.tactics?.selected&&TACTIC_DEFS[pc.tactics.selected]?pc.tactics.selected:coachChooseTactic("Scrim訓練安排"))),ceil=tacticAbilityCeiling(tk),cur=pc.tactics.mastery[tk]||45,room=Math.max(0,ceil-cur),coach=ensureCoachProfile(),eff=((coach?.skills?.tactics||65)/100)*(win?.95:.55),tg=room>0?Math.min(room,Math.max(.1,eff*Math.min(1.35,room/10))):0;pc.tactics.mastery[tk]=clamp(cur+tg,0,100);const others=Object.keys(TACTIC_DEFS).filter(k=>k!==tk).sort((a,b)=>tacticFit(b)-tacticFit(a)),sk=others[0],scur=pc.tactics.mastery[sk]||45,sceil=tacticAbilityCeiling(sk),sg=Math.max(0,Math.min(sceil-scur,tg*.25));if(sg>0)pc.tactics.mastery[sk]=clamp(scur+sg,0,100);pc.coachTrust=clamp((pc.coachTrust||50)+(win?1.2:.35),0,100);state.logs.push(`🆚 Scrim：${pc.team} ${score} ${opp}｜教練安排 ${TACTIC_DEFS[tk].name}${tg>0?` +${tg.toFixed(1)}`:"（已接近陣容上限）"}${sg>0?`，${TACTIC_DEFS[sk].name} +${sg.toFixed(1)}`:""}。`);
...
function contractDetails(c){completeContract(c);const r=c.requirements||{},d=(c.demands||[]);return `出賽率≥${r.appearance??70}%｜賽季${r.season||"前8"}｜風評≥${r.reputation??55}｜Rank ${r.rank||"菁英"}｜違約金 NT$${Number(c.buyout||0).toLocaleString()}${d.length?`<br>已接受附加條款：${d.join("、")}`:""}`}
...
function changeCareerRep(delta,reason){
 const p=state.player;ensureEthics();const serial=state.date.year*52+state.date.week;
 p.adultLife.repPenaltyLedger=p.adultLife.repPenaltyLedger||{};
 const key=String(reason||"未註明").replace(/\d+/g,"#").trim();
 if(delta<0){const prev=p.adultLife.repPenaltyLedger[key];if(prev&&serial-(prev.serial||0)<=8){state.logs.push(`🛡️ 同一職業風評事件「${reason}」再次出現，已阻止重複扣分。`);return false}p.adultLife.repPenaltyLedger[key]={serial,reason,delta};}
 const before=p.adultLife.careerReputation;p.adultLife.careerReputation=clamp(before+delta,0,100);
 p.adultLife.reputationHistory.unshift({delta:+delta.toFixed(1),reason,year:state.date.year,week:state.date.week});
 p.adultLife.reputationHistory=p.adultLife.reputationHistory.slice(0,20);return true;
}
...
function childSupportDecision(name,choice){
 ...
 x.supportChoice=choice;x.child=x.child||{name:`${x.name.slice(0,1)}小星`,gender:(x.childGender||(x.childGender=Math.random()<.5?"男":"女")),birthYear:state.date.year,bond:0,public:false};
...
function childrenPage(){
 ...
 return `<section class="card"><div class="row space"><h2>👶 小孩</h2><span class="badge">${children.length} 位</span></div>${children.length?children.map(x=>{const age=Math.max(0,state.date.year-(x.child.birthYear||state.date.year)),bond=Math.round(x.child.bond||0);return `<div class="schedule-item"><div><strong>${x.child.name}</strong><div class="small">母親：${x.name}｜${(x.child.gender||x.childGender)?`性別：${x.child.gender||x.childGender}｜`:""}${age}歲｜親子關係 ${bond}/100｜扶養：${x.supportChoice||"共同撫養"}｜生產：${x.birthChoice==="陪產"?"有陪產":x.birthChoice==="工作"?"未陪產":"待補登"}</div></div><div><button class="ghost child-care-action" data-name="${x.name}">陪伴孩子</button>${age<=3?`<button class="ghost infant-care-action" data-name="${x.name}">🍼 育嬰照顧</button>`:""}</div></div>`}).join(""):`<div class="notice">目前沒有由你實際照顧的小孩。由媽媽單獨扶養的孩子仍存在於世界資料中，但不會列入你的主要小孩清單。</div>`}</section>
...
function clubSeekSponsor1982(auto=false){const pc=ensureCareer20(),f=pc.clubFinance,lw=ensureLivingWorld1984();lw.clubSponsors=lw.clubSponsors||[];const cap=8;if(lw.clubSponsors.length>=cap){if(!auto)modal(`<h2>🤝 贊助額度已滿</h2><p>${pc.team} 目前已有 ${lw.clubSponsors.length}/${cap} 個戰隊贊助席位。必須等待合約到期，不能無限拉贊助填補財務。</p>${closeBtn()}`);return}const star=starPowerForPlayer(),chance=clamp(.30+star/260+(pc.recentChampions?.length||0)*.025,.28,.82);if(Math.random()<chance){const brands=["曜石科技","北辰電信","星港銀行","極光能源","凌峰汽車","天際航空","Pulse Gear","Vertex Labs"],brand=brands.find(x=>!lw.clubSponsors.some(y=>y.brand===x))||`合作品牌${lw.clubSponsors.length+1}`,amt=Math.round(rand(4500000,15000000)/100000)*100000;lw.clubSponsors.push({brand,amount:amt,expiresYear:state.date.year+(Math.random()<.65?1:2)});f.cash+=amt;f.revenue+=amt;f.sponsorRevenue+=amt;state.logs.push(`🤝 ${pc.team} 簽下 ${brand} 戰隊贊助，取得 NT$${amt.toLocaleString()} 商業資金（${lw.clubSponsors.length}/${cap}）。`);if(!auto){save();render();modal(`<h2>🤝 戰隊贊助</h2><p>${brand} 加入合作，帶來 NT$${amt.toLocaleString()}。目前贊助席位 ${lw.clubSponsors.length}/${cap}。</p>${closeBtn()}`)}}else if(!auto)modal(`<h2>🤝 贊助談判</h2><p>本次沒有品牌願意立即簽約。贊助受戰績、明星度與有限席位限制。</p>${closeBtn()}`)}
...
function npcFanEstimate1991(name,r){const p=state.player,c=state.characters?.[name]||{},rating=r?rosterRating(r):Number(c.rating||c.strength||75),titles=Number(c.worldTitles||c.worldChampionships||c.championships||0),star=Number(c.starPower||c.fans?Math.min(100,Math.log10(Math.max(10,c.fans||0))*18):0);if(name===p.name)return Math.max(Number(p.followers||0),titles*650000);return Math.max(Number(c.fans||c.followers||0),Math.round((rating-60)*18000+titles*520000+star*9000));}
function clubMerchWeeklyTick1982(){const p=state.player,pc=ensureCareer20(),f=pc.clubFinance;ensureAllRosterMerch1985();const activeNames=new Set((pc.roster||[]).map(x=>x.name));f.clubMerch=(f.clubMerch||[]).filter((m,i,a)=>m?.player&&activeNames.has(m.player)&&a.findIndex(z=>z.player===m.player)===i);for(const m of f.clubMerch){const r=(pc.roster||[]).find(x=>x.name===m.player),rating=r?rosterRating(r):(m.player===p.name?avg():75),fans=npcFanEstimate1991(m.player,r),c=state.characters?.[m.player]||{},titles=Number(c.worldTitles||c.worldChampionships||c.championships||0)+(m.player===p.name?Number(pc.worldTitles||0):0),star=m.player===p.name?starPowerForPlayer():Number(c.starPower||Math.min(100,Math.log10(Math.max(10,fans))*14));const demand=1+Math.log10(Math.max(100,fans))/2.4+titles*.32+star/115+Math.max(0,rating-80)/45,units=Math.max(20,Math.round(rand(70,180)*demand)),gross=units*rand(850,2100);m.units=(m.units||0)+units;m.revenue=(m.revenue||0)+gross;m.fans=fans;f.cash+=gross;f.revenue+=gross;f.merchRevenue+=gross;if(m.player===p.name){const share=Math.round(gross*(pc.merchShare||.1));p.cash+=share;f.cash=Math.max(0,f.cash-share);p.proCareer.finance?.history?.unshift({type:`${pc.team}選手周邊分紅`,amount:share,week:state.date.week})}}}
function clubPrestige1991(){const p=state.player,pc=ensureCareer20(),hist=state.world?.recentChampions||pc.recentChampions||[],worlds=hist.filter(x=>/世界|World/i.test(JSON.stringify(x))&&JSON.stringify(x).includes(pc.team)).length+Number(pc.worldTitles||0),msi=hist.filter(x=>/MSI/i.test(JSON.stringify(x))&&JSON.stringify(x).includes(pc.team)).length;return clamp(35+worlds*16+msi*9+starPowerForPlayer()*.35,0,100)}
function clubSponsorCap1991(){const q=clubPrestige1991();return q>=90?16:q>=78?14:q>=65?12:q>=50?10:8}
function ensureNovaOwner1991(){const pc=ensureCareer20();if(pc.team!=="Nova Gaming")return null;pc.owner=pc.owner||{name:"周啟衡",age:48,title:"創辦人兼董事長",relation:68,businessTrust:74,visionFit:66};const o=pc.owner;if(!state.characters[o.name])addSocialAcquaintance(o.name,o.relation,{gender:"男",age:o.age,role:o.title,identityType:"Nova Gaming 老闆／最大股東",occupation:"電競俱樂部董事長",acquaintanceSource:"Nova Gaming 股東會",romanceable:false,homeCountry:"台灣",homeCity:"台北"});return o}
function ownerCard1991(){const o=ensureNovaOwner1991();if(!o||equityPct()<10)return "";return `<section class="card"><h2>🤝 Nova Gaming 董事長｜${o.name}</h2><div class="notice">${o.title}｜私人關係 ${Math.round(o.relation)}/100｜商業信任 ${Math.round(o.businessTrust)}/100｜經營理念 ${Math.round(o.visionFit)}/100<br>夜鋒目前持股 ${equityPct().toFixed(1)}%，可直接與董事長討論重大經營事項。</div><div class="reply-grid"><button class="reply" onclick="ownerInteract1991('future')">討論戰隊未來</button><button class="reply" onclick="ownerInteract1991('roster')">討論陣容配置</button><button class="reply" onclick="ownerInteract1991('contracts')">討論續約／綁定</button><button class="reply" onclick="ownerInteract1991('golf')">⛳ 一起打高爾夫</button></div></section>`}
function ownerInteract1991(kind){const o=ensureNovaOwner1991(),pc=ensureCareer20();if(!o)return;let text="";if(kind==='golf'){if(!consume("與董事長打高爾夫",1))return;o.relation=clamp(o.relation+rand(2,5),0,100);o.businessTrust=clamp(o.businessTrust+rand(1,3),0,100);text=`你與周啟衡打了一場高爾夫。第11洞時，他主動聊到下季預算與核心選手續約。私人關係與商業信任提升。`}else if(kind==='future'){o.visionFit=clamp(o.visionFit+2,0,100);text=`周啟衡與你討論未來三年的方向：維持冠軍窗口、擴大國際商業合作，同時避免薪資結構失控。你的意見已被記入董事會規劃。`}else if(kind==='roster'){const core=(pc.roster||[]).filter(x=>!x.isSub).map(x=>`${x.name}(${x.role}/${rosterRating(x)})`).join('、');text=`你與周啟衡逐一檢視目前先發：${core}。他同意重大出售與補強會先與你討論。`}else{text=`你與周啟衡討論核心選手續約，以及「希望與特定隊友同隊」的共同去留條款。管理層會在後續談判中把搭檔意願列為正式條件。`;ensureContractBonds1991()}state.logs.push(`🏢 與周啟衡：${text}`);save();render();modal(`<h2>🤝 與周啟衡會談</h2><p>${text}</p>${closeBtn()}`)}
function ensureContractBonds1991(){const pc=ensureCareer20();pc.contractBonds=pc.contractBonds||{};for(const x of pc.roster||[]){if(x.isPlayer)continue;const c=state.characters?.[x.name]||{};if(!pc.contractBonds[x.name]&&((state.player.relations?.[x.name]||x.relation||0)>=75||Number(x.chemistry||0)>=78)){pc.contractBonds[x.name]={with:state.player.name,strength:(state.player.relations?.[x.name]||80)>=90?"強烈共同進退":"希望同隊",active:true,createdYear:state.date.year}}}return pc.contractBonds}
function contractBondsCard1991(){if(!isProfessionalStage())return "";const b=ensureContractBonds1991(),rows=Object.entries(b).filter(([,x])=>x?.active);return `<section class="card"><h2>🔗 選手共同去留／合約綁定</h2>${rows.length?rows.map(([n,x])=>`<div class="log"><strong>${n} ↔ ${x.with}</strong><br>${x.strength}｜續約、自由市場與轉會談判會考慮此偏好。</div>`).join(''):`<div class="small">目前沒有形成明確共同去留偏好的選手。高關係與長期默契可能自然形成。</div>`}</section>`}
...
function ensureGlobalProDatabase(){
 ...
 PRO_REGIONS.forEach(region=>{pc.proDatabase[region]=pc.proDatabase[region]||{};regionTeams(region).forEach((team,ti)=>{
   ...
   pc.proDatabase[region][team]=roles.map((role,ri)=>{
     const pool=REGION_PRO_NAMES[region],base=pool[(ti*5+ri)%pool.length],suffixes={LCK:["Jun","Min","Woo","Hyun","Jae","Seok","Tae","Bin","Hwan","Yul","Won","Jin"],LPL:["子謙","承宇","景澤","昊廷","奕辰","宇珩","柏言","曜安","哲熙","睿洋","廷恩","子墨"],LEC:["Vale","Nox","Aster","Rune","Vega","Kiro","Lynx","Miro","Soren","Aero","Riven","Orin"],LCS:["Ray","Cole","Dean","Miles","Blake","Grant","Kai","Reed","Finn","Jace","Noel","Lane"],PCS:["承曜","奕辰","柏勳","宇謙","知衡","昊恩","以安","子昂","言澈","哲宇","允澤","景曜"]},name=ti<2?base:`${base.split(" ")[0]}·${suffixes[region][ti%suffixes[region].length]}`;
...
function annualNpcCareerLifecycle(year){
 ...
 Object.values(db).forEach(region=>Object.values(region).forEach(roster=>roster.forEach(x=>{
   ...
   if(!Number.isFinite(x.birthYear))x.birthYear=year-(Number(x.age)||20);x.age=Math.max(0,year-x.birthYear);
...
function syncCanonicalAges(){
 const p=state.player,year=Number(state.date?.year||2026);
 // Nightblade has a canonical story timeline: 2026=16. Never increment this in another annual path.
 p.birthYear=2010;p.age=Math.max(16,16+(year-2026));
 Object.values(state.characters||{}).forEach(c=>{
   if(!c||!c.name)return;
   const d=String(c.desc||"");
   // Canonical school relationships keep their fixed age gap to Nightblade.
   if(c.name==="林雨晴"||c.name==="許安然"||/同班同學|國中同學|同屆同學|高中好友|同班好友/.test(d)){
     c.birthYear=2010;c.age=p.age;return;
   }
   if(/學妹|高一/.test(d)){c.birthYear=2011;c.age=Math.max(0,year-c.birthYear);return}
   if(/學姊|高三/.test(d)){c.birthYear=2009;c.age=Math.max(0,year-c.birthYear);return}
   // Every persistent NPC gets one stable birth year, then age is derived from it every season.
   if(!Number.isFinite(c.birthYear)){
     const knownAge=Number(c.age);
     if(Number.isFinite(knownAge)&&knownAge>=0)c.birthYear=year-knownAge;
   }
   if(Number.isFinite(c.birthYear))c.age=Math.max(0,year-c.birthYear);
 });
 // Global professionals use the same birth-year rule, preventing the former +2/year path.
 const db=p.proCareer?.proDatabase;
 if(db)Object.values(db).forEach(reg=>Object.values(reg||{}).forEach(ro=>(ro||[]).forEach(x=>{
   if(!Number.isFinite(x.birthYear)&&Number.isFinite(x.age))x.birthYear=year-x.age;
   if(Number.isFinite(x.birthYear))x.age=Math.max(0,year-x.birthYear);
 })));
 if(typeof ensureAster1987==="function"&&isProfessionalStage())ensureAster1987();
}
...
function startPreMatchMedia(){
 ...
 syncTacticLineup1994("正式比賽前陣容");const tactic=coachChooseTactic("正式比賽賽前");const coach=ensureCoachProfile(),fit=tacticFit(tactic);
...
function runRichLeagueMatch(){prepareRuptureEmergencyLineup();const lw1984=ensureLivingWorld1984(),difficultyPressure=.035+Math.max(0,(state.player.proCareer?.careerStats?.seriesW||0)-(state.player.proCareer?.careerStats?.seriesL||0))*.0025+(lw1984.studyPressure||0)*.0011+ruptureMatchPenalty()+Number(state.player.proCareer?.emergencyLineupPenalty||0);
 ...
 while(my<need&&his<need){games++;if(games>1){const coach=ensureCoachProfile(),behind=my<his,adjustRoll=(coach?.skills?.adjust||65)+rand(-18,18);if(behind&&adjustRoll>=72){const before=pc.tactics.selected,next=coachChooseTactic(`GAME ${games} 局間調整`);if(next!==before)logs.push(`🧑‍🏫 GAME ${games} 前｜教練臨場調整：${TACTIC_DEFS[before].name} → ${TACTIC_DEFS[next].name}`)}else logs.push(`🧑‍🏫 GAME ${games} 前｜教練決定維持 ${TACTIC_DEFS[pc.tactics.selected].name}`)}const liveTacticAdj=tacticMatchAdjustment(),realismAdj=realismMatchAdjustment1985(opp,games),wc=clamp((.50+(avg()-68)*.012+(p.condition.form-60)*.003+(chem-50)*.002+inj+lifeAdj+metaAdj+majorAdj+liveTacticAdj+realismAdj.total)-.105-difficultyPressure,.08,.64),win=Math.random()<wc;if(realismAdj.note)logs.push(`🧠 GAME ${games}｜${realismAdj.note}`);if(win)my++;else his++;logs.push(...richGameEvents(games,opp,win))}
 ...
 pc.pendingPostInterview=true;
 updateTeamRelationsAfterMatch(my>his);updateChemistryAfterMatch1985(my>his);updateProfessionalReputation(my>his,mvp);awardMatchPopularity(my>his,mvp,sn.phase==="季後賽"?`${sn.seasonName}季後賽`:"職業聯賽");
...
function makeTransferListing(name,team,region,role,rating,reason,isPlayer=false,source="一軍"){
 ...
 const x={id:`TL-${state.date.year}-${state.date.week}-${Date.now()}-${rand(10,99)}`,name,team,region,role:normalizeRole(role)||role||"中路",rating,reason,asking,status:"掛牌中",listedWeek:state.date.week,isPlayer,source,destination:null,buyer:null,buyerRegion:null,soldWeek:null};if(isPlayer){const pc=state.player.proCareer;x.preTransferContract=pc.contract?JSON.parse(JSON.stringify(pc.contract)):null;x.preTransferEquity={pct:Number(pc.clubEquity?.shares?.[team]||0),costBasis:Number(pc.clubEquity?.costBasis?.[team]||0)};x.preTransferManagementRelation=pc.managementRelation;}
...
function resolveTransferListings(){
 if(!isProfessionalStage())return;const pc=state.player.proCareer,tm=ensureTransferMarket();
 (tm.listings||[]).filter(x=>x.status==="掛牌中").forEach(x=>{
   ...
   if(x.isPlayer&&(hasMajorTransferConsultClause()||Number(pc.clubEquity?.shares?.[x.team]||0)>0)){
     if(pc.pendingMajorTransfer||x.pendingConsultation)return;x.pendingConsultation=true;x.status="等待選手溝通";x.buyer=b.team;x.buyerRegion=b.region;x.destination=toAcademy?"次級聯賽／二隊":"一軍";
     pc.pendingMajorTransfer={listingId:x.id,from:x.team,buyer:b.team,buyerRegion:b.region,toAcademy,sharePct:Number(pc.clubEquity?.shares?.[x.team]||0),year:state.date.year,week:state.date.week};
     state.messages.push({id:`major-transfer-${Date.now()}`,from:`${x.team} 管理層`,text:`收到 ${b.team} 的重大轉會報價。依合約約定，交易已暫停並先通知你；請到生涯頁處理轉會溝通。`,unread:true,resolved:true,type:"contract"});state.logs.push(`📨 重大轉會事前告知：${x.team} → ${b.team}。在夜鋒完成溝通前不得成交。`);return;
   }
...
function dynamicWorldWeeklyTick1967(){npcDevelopmentTick();ensureTwinLinks();maybeMediaContact();seasonOpeningInterviewTick();proHealthTick();livingWorldWeeklyTick1984()}
...
// ===== V1.9.8.5 競爭時代・活世界基礎 =====
const RHYTHM_META_1984={
 early:{name:"前期碰撞版本",desc:"平均節奏加快；打野、輔助與前15分鐘主動性更重要。"},
 jungle:{name:"野核／野區版本",desc:"野區資源與打野發育權重提高。"},
 bot:{name:"下路大核版本",desc:"下路資源與雙C保護成為主流。"},
 top:{name:"上半區版本",desc:"上野碰撞、先鋒與邊線優勢更關鍵。"},
 teamfight:{name:"正面團戰版本",desc:"龍魂、大龍與正面5v5權重提高。"},
 macro:{name:"營運／物件版本",desc:"轉線、視野與物件交換比單純打架更重要。"},
 split:{name:"邊線分推版本",desc:"單帶、131與兵線調度價值提高。"}
};
function ensureLivingWorld1984(){const pc=ensureCareer20();pc.livingWorld=pc.livingWorld||{};const w=pc.livingWorld;w.memories=Array.isArray(w.memories)?w.memories:[];w.worldFeed=Array.isArray(w.worldFeed)?w.worldFeed:[];w.classics=Array.isArray(w.classics)?w.classics:[];w.tvHistory=Array.isArray(w.tvHistory)?w.tvHistory:[];w.npcLinks=w.npcLinks||{};w.tacticExposure=w.tacticExposure||{};w.studyPressure=Number(w.studyPressure||0);w.lastTickKey=w.lastTickKey||"";if(!w.rhythm||w.rhythm.year!==state.date.year||Math.abs((w.rhythm.week||0)-state.date.week)>=8){const keys=Object.keys(RHYTHM_META_1984),key=keys[stableAgeOffset(`${state.date.year}-${Math.floor((state.date.week-1)/8)}-rhythm`,keys.length)];w.rhythm={key,name:RHYTHM_META_1984[key].name,desc:RHYTHM_META_1984[key].desc,year:state.date.year,week:state.date.week}}return w}
function championshipStudyPressure1984(){const pc=state.player.proCareer,a=state.player.achievements||[],intl=a.filter(x=>/世界賽冠軍|MSI冠軍/.test(x.title||x.name||"")).length,dom=a.filter(x=>/春季賽冠軍|夏季賽冠軍/.test(x.title||x.name||"")).length,net=Math.max(0,(pc.careerStats?.seriesW||0)-(pc.careerStats?.seriesL||0));return clamp(8+intl*10+dom*3+net*.32,5,82)}
function remember1984(who,type,text,weight=1){const w=ensureLivingWorld1984();w.memories.unshift({who,type,text,weight,year:state.date.year,week:state.date.week});w.memories=w.memories.slice(0,80);const c=state.characters?.[who];if(c){c.memories=Array.isArray(c.memories)?c.memories:[];c.memories.unshift({type,text,year:state.date.year,week:state.date.week});c.memories=c.memories.slice(0,12)}}
function npcAutonomy1984(){const w=ensureLivingWorld1984(),known=Object.values(state.characters||{}).filter(c=>c?.known&&c.name!==state.player.name);if(!known.length)return;const c=known[stableAgeOffset(`${state.date.year}-${state.date.week}-life`,known.length)],jobs=c.occupation||c.career||c.identityType||"一般人物",events=[`${c.name} 最近把重心放在自己的${/選手|教練|主播|主持|記者/.test(jobs)?"職涯":"生活目標"}上。`,`${c.name} 與自己的朋友圈有了新的往來，生活並沒有停在等待夜鋒。`,`${c.name} 最近開始規劃下一階段的人生方向。`];const text=events[stableAgeOffset(c.name+state.date.week,events.length)];w.worldFeed.unshift({type:"人物",text,year:state.date.year,week:state.date.week});if(Math.random()<.22)remember1984(c.name,"自主生活",text,.5)}
function npcRelationshipNetworkTick1984(){const w=ensureLivingWorld1984(),arr=Object.values(state.characters||{}).filter(c=>c?.known&&c.name!==state.player.name);if(arr.length<2||Math.random()>.28)return;const a=arr[rand(0,arr.length-1)],b=arr.filter(x=>x.name!==a.name)[rand(0,arr.length-2)];if(!b)return;const key=[a.name,b.name].sort().join("|");w.npcLinks[key]=w.npcLinks[key]||{a:a.name,b:b.name,relation:20+stableAgeOffset(key,45),type:"認識"};const l=w.npcLinks[key];l.relation=clamp(l.relation+rand(-2,4),0,100);if(l.relation>=72&&l.type==="認識")l.type="朋友";if(l.relation>=88&&a.gender!==b.gender&&Number(a.age||0)>=18&&Number(b.age||0)>=18&&Math.random()<.12)l.type="曖昧";if(Math.random()<.16)w.worldFeed.unshift({type:"人際",text:`${a.name} 與 ${b.name} 最近因${/選手/.test((a.career||"")+(b.career||""))?"電競圈活動":"共同朋友"}有了更多往來。`,year:state.date.year,week:state.date.week})}
function maybeMentorPath1984(){const p=state.player,pc=p.proCareer,w=ensureLivingWorld1984();if((pc.careerStats?.matches||0)<60||starPowerForPlayer()<55||Object.keys(pc.legacy?.apprentices||{}).length>=3)return;if(w.lastMentorOfferYear===state.date.year||Math.random()>.07)return;const db=ensureGlobalProDatabase(),cands=[];Object.values(db||{}).forEach(reg=>Object.values(reg||{}).forEach(ro=>(ro||[]).forEach(x=>{if(x.active&&!x.retired&&(x.age||99)<=20&&(x.rating||0)>=72&&(x.rating||0)<=96)cands.push(x)})));if(!cands.length)return;const x=cands.sort((a,b)=>(b.potential||b.rating||0)-(a.potential||a.rating||0))[0];w.lastMentorOfferYear=state.date.year;addSocialAcquaintance(x.name,22,{gender:"男",age:x.age||19,career:"職業選手",occupation:"職業選手",identityType:"職業選手",acquaintanceSource:"主動請教夜鋒",traits:["好勝","上進","尊敬前輩"]});state.messages.push({id:`mentor-${Date.now()}`,from:x.name,text:`夜鋒前輩，我一直有研究你的比賽。如果休賽期有機會，我想正式向你請教，可以嗎？`,unread:true,resolved:true,type:"normal"});w.pendingMentor=x.name;w.worldFeed.unshift({type:"新秀",text:`🌱 ${x.age||19}歲新秀 ${x.name} 主動向夜鋒請教，師徒之路出現契機。`,year:state.date.year,week:state.date.week})}
function rivalInteractionTick1984(){const pc=state.player.proCareer,w=ensureLivingWorld1984(),entries=Object.entries(pc.rivals||{});if(!entries.length||Math.random()>.20)return;const [name,r]=entries.sort((a,b)=>(b[1].meetings||0)-(a[1].meetings||0))[0],lines=[`⚔️ ${name} 在節目上表示：「下一次碰到夜鋒，我還是想正面贏一次。」`,`🎮 夜鋒與 ${name} 在高分段Rank再次撞車，社群開始討論兩人的下一次正式交手。`,`🎙️ 媒體重新整理夜鋒與 ${name} 的交手史，宿敵話題再次升溫。`],text=lines[stableAgeOffset(name+state.date.week,lines.length)];w.worldFeed.unshift({type:"宿敵",text,year:state.date.year,week:state.date.week});remember1984(name,"宿敵",text,.8)}
function maybeTVProgram1984(){
 const w=ensureLivingWorld1984(),pc=state.player.proCareer;if(Math.random()>.11)return;
 const shows=["電競夜話","王者聊天室","深夜選手席","賽場之外"],show=shows[stableAgeOffset(`${state.date.year}-${state.date.week}`,shows.length)],host=ensureGoddessHost1983(),now=state.date.year*52+state.date.week;
 w.tvGuestCooldown=w.tvGuestCooldown||{};
 const roster=(pc.roster||[]).filter(x=>!x.isPlayer).map(x=>x.name),rivals=Object.keys(pc.rivals||{}),pool=[state.player.name,...roster,...rivals].filter(Boolean);
 let best=state.player.name,bestScore=-999;
 for(const name of [...new Set(pool)]){const last=Number(w.tvGuestCooldown[name]||-9999),cool=now-last<10?-80:0,isMe=name===state.player.name,rel=state.player.relations?.[name]||50;let score=(isMe?starPowerForPlayer()+18:45+rel*.25)+cool+stableAgeOffset(`${name}-${now}-tv`,24);if(isMe&&(pc.worldChampionYear||state.player.followers>300000))score+=18;if(score>bestScore){bestScore=score;best=name}}
 const actualGuest=best;w.tvGuestCooldown[actualGuest]=now;
 const text=`📺《${show}》本週邀請 ${actualGuest} 談版本與職業生活，由 ${host.name} 主持。`;w.tvHistory.unshift({show,host:host.name,guest:actualGuest,year:state.date.year,week:state.date.week});w.worldFeed.unshift({type:"節目",text,year:state.date.year,week:state.date.week});
 if(actualGuest===state.player.name)state.messages.push({id:`tv-${Date.now()}`,from:`《${show}》製作組`,text:`我們想邀請夜鋒參加本週節目，由 ${host.name} 主持，會談近期版本、戰隊、生涯與宿敵。`,unread:true,resolved:false,type:"mediaEventInvite",eventTopic:`《${show}》節目錄影`});
}
function livingWorldWeeklyTick1984(){if(!isProfessionalStage())return;const w=ensureLivingWorld1984(),key=`${state.date.year}-${state.date.week}`;if(w.lastTickKey===key)return;w.lastTickKey=key;w.studyPressure=championshipStudyPressure1984();const tk=state.player.proCareer.tactics?.selected||"midjungle";w.tacticExposure[tk]=clamp(Number(w.tacticExposure[tk]||0)+.55,0,20);Object.keys(w.tacticExposure).forEach(k=>{if(k!==tk)w.tacticExposure[k]=Math.max(0,w.tacticExposure[k]-.18)});npcAutonomy1984();npcRelationshipNetworkTick1984();maybeMentorPath1984();rivalInteractionTick1984();maybeTVProgram1984();w.clubSponsors=(w.clubSponsors||[]).filter(x=>(x.expiresYear||9999)>=state.date.year);w.worldFeed=w.worldFeed.slice(0,35)}
function livingWorldCard1984(){if(!isProfessionalStage())return"";const w=ensureLivingWorld1984(),rh=RHYTHM_META_1984[w.rhythm.key]||RHYTHM_META_1984.early,feed=w.worldFeed.slice(0,6),mem=w.memories.slice(0,3);return `<section class="card"><div class="row space"><h2>🌍 活的電競世界</h2><span class="badge">被研究 ${Math.round(w.studyPressure||0)}%</span></div><div class="notice"><strong>⚡ 當前節奏：${rh.name}</strong><br>${rh.desc}<br>冠軍與長期強勢會提高對手對 ${state.player.proCareer.team} 的研究程度；重複使用同一體系也更容易被破解。</div>${feed.length?`<div class="log">${feed.map(x=>x.text).join("<br><br>")}</div>`:`<div class="small">世界事件會隨每週推進累積。</div>`}${mem.length?`<details><summary>🧠 NPC近期記憶</summary><div class="small">${mem.map(x=>`${x.who}｜${x.text}`).join("<br>")}</div></details>`:""}<div class="reply-grid"><button id="tvWorld1984" class="reply">📺 電競節目／圈內動態</button><button id="rivalWorld1984" class="reply">⚔️ 宿敵與新秀</button></div></section>`}
function showTVWorld1984(){const w=ensureLivingWorld1984(),rows=w.tvHistory.slice(0,8);modal(`<h2>📺 電視台／網路節目</h2>${rows.length?`<div class="log">${rows.map(x=>`${x.year} W${x.week}｜《${x.show}》｜主持 ${x.host}｜來賓 ${x.guest}`).join("<br><br>")}</div>`:`<p>目前尚未累積節目紀錄；節目會邀請夜鋒、宿敵、新秀、隊友與其他明星，不只圍繞主角。</p>`}${closeBtn()}`)}
function showRivalWorld1984(){const pc=state.player.proCareer,w=ensureLivingWorld1984(),rs=Object.entries(pc.rivals||{}).slice(0,8),ap=Object.keys(pc.legacy?.apprentices||{});modal(`<h2>⚔️ 宿敵／新秀／師徒</h2><div class="notice">師徒不是按鈕硬解鎖：職業資歷、明星度與成績足夠後，年輕選手會主動請教。你目前${w.pendingMentor?`已收到 ${w.pendingMentor} 的請教契機`:`符合條件時會自然觸發`}。</div>${rs.length?`<div class="log">${rs.map(([n,r])=>`${n}｜正式交手 ${r.meetings||0} 次｜競技宿敵`).join("<br>")}</div>`:""}${ap.length?`<div class="notice">🎓 現有徒弟：${ap.join("、")}</div>`:""}${closeBtn()}`)}
function contextualInterviewQuestions1984(ctx,host,opp){const pc=state.player.proCareer,w=ensureLivingWorld1984(),phase=proAnnualPhase(),bo5=/季後賽|MSI|世界賽/.test(ctx.title+phase),rival=pc.rivals?.[opp],qs=[];if(bo5)qs.push({q:`${host.name}：「如果要你預測這個BO5的比分，你會選哪一個？」`,a:[["3-0","3：0"],["3-1","3：1"],["3-2","3：2"],["no-score","不預測比分"]]});const liveMeta=ensureMeta(),rhythm=RHYTHM_META_1984[w.rhythm.key]?.name||"一般節奏";qs.push({q:`${host.name}：「目前遊戲版本主軸是『${liveMeta.style}』，賽場節奏呈現『${rhythm}』，你覺得 ${pc.team} 的適應如何？」`,a:[["meta-ready","我們準備得很好"],["meta-adjust","還在調整，但會找到答案"],["meta-team","版本只是條件，執行更重要"]]});if(rival)qs.push({q:`${host.name}：「你和 ${opp} 已經多次交手，這場對你來說是不是宿敵戰？」`,a:[["rival-respect","我尊重這個對手"],["rival-fire","我就是想再贏他一次"],["rival-calm","歷史不會替今天打比賽"]]});qs.push({q:`${host.name}：「外界認為冠軍隊已經被大量研究，你們會不會擔心原本的招牌戰術失效？」`,a:[["change","我們準備了新的東西"],["trust","被研究也不代表能阻止我們"],["team","臨場調整才是關鍵"]]});return qs.slice(0,5)}
function migrateProV1984(){const p=state.player;if(p.v1984Migrated)return;if(isProfessionalStage()){const w=ensureLivingWorld1984();w.studyPressure=championshipStudyPressure1984();const pc=p.proCareer;pc.clubFinance.clubMerch=pc.clubFinance.clubMerch||[];const roster=(pc.roster||[]).filter(x=>!x.isSub);roster.forEach((x,i)=>{if(!pc.clubFinance.clubMerch.some(m=>m.player===x.name))pc.clubFinance.clubMerch.push({id:`roster-merch-1984-${i}-${x.name}`,name:`${x.name} 官方選手系列`,player:x.name,launchedYear:state.date.year,units:0,revenue:0,active:true})});if(!pc.clubFinance.clubMerch.some(m=>m.player===p.name))pc.clubFinance.clubMerch.unshift({id:"player-merch-1984",name:`${p.name} 官方選手系列`,player:p.name,launchedYear:state.date.year,units:0,revenue:0,active:true});state.logs.push("🆕 V1.9.8.5：競爭時代・活世界基礎啟用。已接通版本節奏、冠軍隊被研究、戰術克制、情境採訪、NPC自主生活／記憶／關係網、師徒契機、宿敵動態、節目、贊助上限與全隊周邊。")}p.v1984Migrated=true}


// ===== V1.9.8.5 真實職業世界 =====
function ensureRealism1985(){const pc=ensureCareer20();pc.realism=pc.realism||{};const r=pc.realism;r.formCurve=Number.isFinite(r.formCurve)?r.formCurve:0;r.chemistry=r.chemistry||{midJungle:55,botLane:55,jungleSupport:55,teamfight:58,communication:60,trust:60};r.intel=r.intel||{scrimLeakRisk:8,opponentStudy:0,lastLeak:null};r.scouting=r.scouting||{uncertainty:true};r.openingInterview=r.openingInterview||null;r.mentorStories=r.mentorStories||{};r.rivalStories=r.rivalStories||{};r.careerDesires=r.careerDesires||{};return r}
function personalityTraits1985(x){if(!x)return[];return Array.isArray(x.traits)?x.traits:safeTraits(x)}
function personalityMatchAdjustment1985(opp,gameNo){const p=state.player,pc=p.proCareer,r=ensureRealism1985(),mine=personalityTraits1985(p),roster=(pc.roster||[]).filter(x=>!x.isSub),all=[mine,...roster.map(personalityTraits1985)].flat(),oppStrength=rosterRating(opp)||76,myStrength=avg(),weak=oppStrength<myStrength-9,decider=gameNo>=5||pc.season?.phase==="季後賽"&&gameNo>=4;let adj=0,notes=[];if(weak&&all.includes("自大")&&Math.random()<.38){adj-=.055;notes.push("隊內出現輕敵情緒，前期決策變得冒進")};if(all.includes("懶惰"))adj-=.012;if(all.includes("急性子")&&p.stress>55){adj-=.018;notes.push("壓力下有人急於強開找機會")};if(all.includes("耍大牌")&&pc.lockerRoom<60)adj-=.02;if(all.includes("玻璃心")&&p.condition.form<55)adj-=.025;if(decider&&all.includes("大心臟")){adj+=.028;notes.push("大心臟選手在關鍵局更敢做決策")};if(decider&&all.includes("冷靜"))adj+=.012;if(all.includes("團隊型"))adj+=.009;return {adj,note:notes.join("；")}}
function chemistryAdjustment1985(){const c=ensureRealism1985().chemistry;return clamp(((c.midJungle+c.botLane+c.jungleSupport+c.teamfight+c.communication+c.trust)/6-55)*.0017,-.07,.08)}
function realismMatchAdjustment1985(opp,gameNo){const r=ensureRealism1985(),pa=personalityMatchAdjustment1985(opp,gameNo),form=clamp(r.formCurve*.003,-.06,.06),chem=chemistryAdjustment1985(),study=clamp((r.intel.opponentStudy||0)*-.0012,-.07,0);return {total:pa.adj+form+chem+study,note:pa.note||((r.intel.opponentStudy||0)>35?"對手針對近期招牌打法做了大量研究":"")}}
function careerDesire1985(x){const r=ensureRealism1985();if(r.careerDesires[x.name])return r.careerDesires[x.name];const opts=["高薪","冠軍","穩定先發","核心地位","留在家鄉","海外挑戰","家庭生活","商業曝光"],a=opts[stableAgeOffset(x.name+"desire",opts.length)],b=opts[stableAgeOffset(x.name+"desire2",opts.length)];return r.careerDesires[x.name]=[a,...(a===b?[]:[b])]}
function weeklyRealismTick1985(){if(!isProfessionalStage())return;const p=state.player,pc=p.proCareer,r=ensureRealism1985();r.formCurve=clamp(r.formCurve+(Math.random()-.5)*4+(p.mood-60)*.015-(p.stress-45)*.018,-20,20);const c=r.chemistry,base=pc.lockerRoom>=65?.6:-.35;Object.keys(c).forEach(k=>c[k]=clamp(c[k]+base+(Math.random()-.5)*1.2,20,98));r.intel.opponentStudy=clamp((ensureLivingWorld1984().studyPressure||0)+(ensureLivingWorld1984().tacticExposure?.[pc.tactics?.selected||"midjungle"]||0)*2,0,85);if(Math.random()<(r.intel.scrimLeakRisk||8)/100*.08){r.intel.lastLeak=`W${state.date.week} 訓練賽情報遭圈內人士討論`;ensureLivingWorld1984().worldFeed.unshift({type:"情報",text:`🕵️ ${pc.team} 的部分訓練賽風格遭外界掌握，近期對手研究度上升。`,year:state.date.year,week:state.date.week});r.intel.opponentStudy=clamp(r.intel.opponentStudy+8,0,90)};(pc.roster||[]).forEach(x=>{careerDesire1985(x);if(!x.traits||!x.traits.length)x.traits=fallbackTraits(x)});storyMentorTick1985();storyRivalTick1985();ensureAllRosterMerch1985()}
function ensureAllRosterMerch1985(){const pc=ensureCareer20(),f=pc.clubFinance,players=[{name:state.player.name},...(pc.roster||[]).filter(x=>!x.isSub)];players.forEach((x,i)=>{if(!f.clubMerch.some(m=>m.player===x.name&&m.active))f.clubMerch.push({id:`auto1985-${state.date.year}-${i}-${x.name}`,name:`${x.name} 官方選手系列`,player:x.name,launchedYear:state.date.year,units:0,revenue:0,active:true})})}
function storyMentorTick1985(){
 const pc=state.player.proCareer,r=ensureRealism1985(),w=ensureLivingWorld1984(),name=w.pendingMentor;if(!name)return;
 const st=r.mentorStories[name]||(r.mentorStories[name]={stage:0,trust:0});
 if(st.stage<0)return;
 if(st.stage===0){
   st.stage=.5;
   state.messages.push({id:`mentor-story-${Date.now()}`,from:name,text:"前輩，我不想只是問一個技巧。我最近在職業賽一直卡住，如果有空，能不能看一次我的比賽再告訴我問題在哪？",unread:true,resolved:false,type:"mentorStory"});
   remember1984(name,"師徒契機",`${name} 第一次提出具體請教，等待夜鋒回應。`,1);return;
 }
 if(!st.playerAccepted||st.stage<1)return;
 if(st.nextStoryWeek&&state.date.week<st.nextStoryWeek)return;
 if(st.stage<4&&Math.random()<.30){
   st.stage++;st.trust+=rand(10,18);st.nextStoryWeek=state.date.week+rand(2,5);
   const lines={2:`${name} 把最近兩場比賽紀錄傳給夜鋒。夜鋒指出他的路線規劃太依賴操作優勢。`,3:`${name} 在正式比賽嘗試調整節奏；有幾波成功，也有一次因過度相信操作而送掉物件。賽後他主動回來討論。`,4:`${name} 經過一段時間反覆請教與實戰驗證後，正式提出希望把夜鋒視為職涯導師。`};
   const txt=lines[Math.floor(st.stage)]||`${name} 與夜鋒持續進行實戰型交流。`;
   w.worldFeed.unshift({type:"師徒",text:`🎓 ${txt}`,year:state.date.year,week:state.date.week});remember1984(name,"師徒故事",txt,2);
   if(st.stage>=4){pc.legacy.apprentices[name]={name,progress:25,sinceYear:state.date.year,storyEstablished:true};const c=state.characters?.[name];if(c)c.identityType="職業選手・徒弟";w.pendingMentor=null;state.logs.push(`🎓 經過請教、觀賽、實戰驗證與長期交流，${name} 正式與夜鋒建立師徒關係。`)}
 }
}
function storyRivalTick1985(){const pc=state.player.proCareer,r=ensureRealism1985(),entries=Object.entries(pc.rivals||{});if(!entries.length||Math.random()>.28)return;const [name,rv]=entries.sort((a,b)=>(b[1].meetings||0)-(a[1].meetings||0))[0],st=r.rivalStories[name]||(r.rivalStories[name]={heat:20,chapters:0});st.heat=clamp(st.heat+rand(1,5),0,100);st.chapters++;const lines=[`⚔️ ${name} 在賽後記者會點名夜鋒：「我不想只當背景，我要在BO5正面贏他。」`,`🔥 夜鋒與 ${name} 的Rank碰撞引爆討論，兩邊粉絲開始翻出歷年正式交手紀錄。`,`🎙️ 節目組安排 ${name} 回看與夜鋒的經典對局，他公開指出下一次會針對夜鋒的習慣。`,`🧩 ${name} 的戰隊近期明顯加入針對夜鋒招牌英雄與中路節奏的設計。`],text=lines[st.chapters%lines.length];ensureLivingWorld1984().worldFeed.unshift({type:"宿敵故事",text,year:state.date.year,week:state.date.week});remember1984(name,"宿敵故事",text,1.5)}
function seasonOpeningInterviewCard1985(){if(!isProfessionalStage())return"";const pc=state.player.proCareer,r=ensureRealism1985(),iv=r.openingInterview;if(!iv||iv.year!==state.date.year||iv.done)return"";return `<section class="card"><h2>🎙️ ${state.date.year} 開季專訪</h2><div class="notice">聯賽媒體正在等待 ${pc.team} 公開本季目標。這次不會自動替夜鋒選答案。</div><button id="openingInterview1985" class="reply">進行完整開季專訪</button></section>`}
function startSeasonOpeningInterview1985(){const pc=state.player.proCareer,r=ensureRealism1985(),host=ensureMediaHost();r.openingInterview={year:state.date.year,done:false,host:host.name,step:0,answers:[]};showSeasonOpeningInterview1985()}
function showSeasonOpeningInterview1985(){const pc=state.player.proCareer,r=ensureRealism1985(),iv=r.openingInterview,qs=[{q:`${iv.host}：「新賽季開始，${pc.team} 今年最重要的目標是什麼？」`,a:[["domestic","先拿下聯賽冠軍"],["international","國際賽冠軍"],["world","世界冠軍"],["growth","先把隊伍磨合好"]]},{q:`${iv.host}：「外界已經開始研究你們的打法，今年會做出多大改變？」`,a:[["change","我們準備了新的東西"],["trust","招牌打法被研究也能贏"],["adapt","會依版本持續調整"]]},{q:`${iv.host}：「如果今年沒有奪冠，你會認為這是一個失敗的賽季嗎？」`,a:[["yes","對冠軍隊而言，是"],["process","要看我們是否持續進步"],["team","結果由整支隊伍承擔"]]}],q=qs[iv.step];if(!q){iv.done=true;pc.publicSeasonGoal=iv.answers[0]||"growth";state.logs.push(`🎙️ ${state.date.year} 開季專訪完成，由 ${iv.host} 主持；公開目標由夜鋒親自回答。`);save();render();return modal(`<h2>🎙️ 開季專訪完成</h2><p>${iv.host}：「謝謝夜鋒。接下來就看 ${pc.team} 今年能走多遠。」</p>${closeBtn()}`)}modal(`<h2>🎙️ 開季專訪 ${iv.step+1}/3</h2><p>${q.q}</p><div class="reply-grid">${q.a.map(a=>`<button class="reply opening-answer1985" data-v="${a[0]}">${a[1]}</button>`).join("")}</div>`);document.querySelectorAll(".opening-answer1985").forEach(b=>b.onclick=()=>{iv.answers.push(b.dataset.v);iv.step++;document.querySelector(".modal-backdrop")?.remove();showSeasonOpeningInterview1985()})}
function updateChemistryAfterMatch1985(win){const c=ensureRealism1985().chemistry,d=win?rand(1,3)*.35:-rand(1,2)*.3;["midJungle","botLane","jungleSupport","teamfight","communication","trust"].forEach(k=>c[k]=clamp(c[k]+d,20,98))}
function playerCareerStats1985(){const pc=state.player.proCareer,h=pc.matchHistory||[],bo5=h.filter(x=>{const sc=String(x.score||"").split(":").map(Number);return Math.max(...sc)>=3}),wins=h.filter(x=>x.win).length;return {series:h.length,wins,winRate:h.length?Math.round(wins/h.length*1000)/10:0,bo5:bo5.length,bo5wins:bo5.filter(x=>x.win).length,mvp:pc.careerStats?.mvp||0}}
function ageCurve1985(x){const age=Number(x.age||state.player.age||22),types=["早熟","正常","晚成","長青","曇花型"],type=x.developmentType||(x.developmentType=types[stableAgeOffset(x.name+"curve",types.length)]);let trend=type==="早熟"?(age<=21?"高速成長":age>=25?"提早下滑":"巔峰"):type==="晚成"?(age<22?"緩慢成長":age<=27?"成長加速":"成熟期"):type==="長青"?(age<24?"穩定成長":"維持力佳"):type==="曇花型"?(age<=22?"爆發成長":"波動較大"):age<23?"成長期":age<27?"巔峰期":"經驗轉型";return {type,trend}}
function scoutSummary1985(x){const rt=Math.round(rosterRating(x)||70),age=ageCurve1985(x),confidence=55+stableAgeOffset(x.name+"scout",36),growth=rt>=90?"已接近頂尖":x.age<=20?"仍有明顯成長空間":"成長空間需觀察";return `${x.name}｜目前評估 ${rt}｜${age.type}・${age.trend}｜${growth}｜球探信心 ${confidence}%`}
function realismCareerCard1985(){if(!isProfessionalStage())return"";const pc=state.player.proCareer,r=ensureRealism1985(),c=r.chemistry,form=r.formCurve>=10?"🔥 火熱":r.formCurve<=-10?"😵 低潮":r.formCurve>=4?"📈 上升":"正常",des=(pc.roster||[]).filter(x=>!x.isSub).slice(0,5),cs=playerCareerStats1985();return `${seasonOpeningInterviewCard1985()}<section class="card"><h2>🧬 真實競技層</h2><div class="stat-grid">${stat("狀態曲線",form)}${stat("團戰默契",Math.round(c.teamfight))}${stat("溝通信任",Math.round((c.communication+c.trust)/2))}${stat("對手研究",Math.round(r.intel.opponentStudy||0)+"%")}</div><div class="small">能力值是長期實力，不等於每場發揮。個性、狀態、傷病、隊伍化學反應、版本節奏、戰術適配與對手研究都會實際進入比賽計算。</div><details><summary>📊 生涯數據中心</summary><div class="small">系列賽 ${cs.series}｜勝率 ${cs.winRate}%｜BO5 ${cs.bo5wins}勝/${cs.bo5}場｜MVP ${cs.mvp}</div></details>${des.length?`<details><summary>🎯 隊友職涯需求／球探觀察</summary><div class="small">${des.map(x=>`${x.name}：${careerDesire1985(x).join("／")}<br>${scoutSummary1985(x)}`).join("<br><br>")}</div></details>`:""}</section>`}
function migrateProV1985(){const p=state.player;if(p.v1985Migrated)return;if(isProfessionalStage()){const pc=ensureCareer20(),r=ensureRealism1985();/* 清除舊版一鍵建立的師徒，只保留未來有故事旗標者 */Object.keys(pc.legacy?.apprentices||{}).forEach(n=>{const a=pc.legacy.apprentices[n];if(!a?.storyEstablished){delete pc.legacy.apprentices[n];const c=state.characters?.[n];if(c&&/徒弟/.test(c.identityType||""))c.identityType="職業選手"}});/* 開季專訪改成待玩家親自作答 */if(state.date.week<=8)r.openingInterview={year:state.date.year,done:false,step:0,answers:[]};/* 舊小孩補上固定且可保存的性別 */(p.adultLife?.pregnancies||[]).filter(x=>x.born).forEach(x=>{if(!x.childGender)x.childGender=stableAgeOffset((x.eventId||x.name)+"gender",2)?"女":"男"});ensureAllRosterMerch1985();state.logs.push("🆕 V1.9.8.5：真實職業世界啟用。舊版一鍵師徒已解除；個性／狀態／化學反應／對手研究正式影響比賽，開季專訪改為完整問答，全隊周邊自動生成，小孩補上性別。")}p.v1985Migrated=true}


function ensureAster1987(){
 if(!isProfessionalStage())return null;const p=state.player,pc=ensureCareer20(),year=Number(state.date.year||2028);
 pc.specialPros=pc.specialPros||{};let a=pc.specialPros.aster;
 if(!a)a=pc.specialPros.aster={name:"江承曜",id:"Aster",debutYear:year,birthYear:year-17,team:"KNG Esports",region:"PCS",role:"打野",rating:94,potential:99,rankPoints:2658,active:true,retired:false,contractYears:2,salary:360000,traits:["大心臟","競爭心","上進"],growthType:"極高成長",schoolLink:"夜鋒高中母校學弟",rookie:true};
 a.age=Math.max(17,year-a.birthYear);a.team=a.team||"KNG Esports";a.region=fixedTeamRegion(a.team)||a.region||"PCS";
 const db=ensureGlobalProDatabase(),ro=db.PCS?.[a.team];if(ro){let ex=ro.find(x=>x.name===a.name);if(!ex){const old=ro.find(x=>x.role==="打野"&&!x.isSub);if(old)old.isSub=true;ro.unshift({...a})}else Object.assign(ex,a)}
 addSocialAcquaintance(a.name,Math.max(18,p.relations?.[a.name]||0),{gender:"男",age:a.age,birthYear:a.birthYear,career:"職業選手",occupation:"職業選手",identityType:"職業選手",acquaintanceSource:"高中母校學弟／KNG新人",desc:"夜鋒高中母校學弟；高中與業餘重要盃賽幾乎全數奪冠，17歲首年進入職業。",isPro:true,team:a.team,region:a.region,role:"打野",rating:a.rating,rankPoints:a.rankPoints,traits:a.traits});
 const c=state.characters?.[a.name];if(c){c.currentRelationship=c.currentRelationship||"學長學弟";c.homeCountry="台灣";c.homeCity=c.homeCity||"桃園";c.rating=a.rating;c.rankPoints=a.rankPoints;c.birthYear=a.birthYear;c.age=a.age}
 return a;
}
function asterStoryTick1987(){
 if(!isProfessionalStage())return;const pc=ensureCareer20(),a=ensureAster1987(),w=ensureLivingWorld1984();if(!a)return;pc.asterStory=pc.asterStory||{stage:0,meetings:0,respect:45,rivalry:20};const st=pc.asterStory;
 if(st.stage===0){st.stage=1;w.worldFeed.unshift({type:"超級新人",text:`🌟 KNG Esports 宣布17歲打野江承曜「Aster」加入一軍。這位夜鋒母校學弟以約 ${a.rankPoints} 分登上高端榜，高中／業餘重要盃賽保持驚人的冠軍紀錄。`,year:state.date.year,week:state.date.week});state.news.unshift(`🌟 17歲天才打野 Aster（江承曜）加盟 KNG Esports，首年職業生涯正式開始。`);remember1984(a.name,"職業出道",`17歲加盟KNG；能力評估94、Rank約${a.rankPoints}分，是夜鋒高中母校學弟。`,3);return}
 if(st.stage===1&&Math.random()<.10){st.stage=2;state.messages.push({id:`aster-${Date.now()}`,from:a.name,text:"學長，我以前在學校就一直看你的比賽。進職業後我不想只被叫做你的學弟——如果遇到你，我會想正面贏一次。",unread:true,resolved:true,type:"normal"});remember1984(a.name,"學長學弟",`Aster進職業後第一次主動聯絡夜鋒，尊敬與競爭心同時存在。`,2)}
 // Do not predetermine the outcome: repeated meetings can later become mentor, rival, or both.
 const rv=pc.rivals?.[a.name];if(rv&&(rv.meetings||0)>=2){st.meetings=rv.meetings;st.rivalry=clamp(st.rivalry+2,0,100);if(!pc.legacy.rivals[a.name]&&st.rivalry>=35)pc.legacy.rivals[a.name]={name:a.name,meetings:rv.meetings,respect:st.respect,schoolJunior:true}}
}
function migrateProV1987(){
...
function migrateProV1989(){
 ...
 const sold=[...(tm.history||[]),...(tm.listings||[])].filter(x=>x?.isPlayer&&x.status==="已成交"&&x.buyer===pc.team).sort((a,b)=>(b.soldWeek||0)-(a.soldWeek||0));const x=sold[0];
 const refunds=(e.refundHistory||[]).filter(r=>r?.team&&r.team!==pc.team&&/V1\.9\.8\.8|股份回購|轉會/.test(r.reason||""));const rr=refunds[0];
 if(x&&rr&&x.team===rr.team){const wrongTeam=pc.team,old=x.team,oldRegion=x.region||fixedTeamRegion(old)||"PCS",pct=Number(rr.pct||49),refund=Number(rr.refund||0);pc.team=old;pc.region=oldRegion;pc.stage="starter";e.shares[old]=pct;e.costBasis[old]=refund||Number(e.costBasis[old]||0);e.team=old;
   // Reverse the erroneous cash refund without allowing negative cash; any shortfall is absorbed as a system correction, not player debt.
...
function migrateProV1993(){const p=state.player;if(p.v1993Migrated)return;if(isProfessionalStage()){ensureSecondLife1993();const pc=ensureCareer20();pc.tactics.trainingFocus=pc.tactics.trainingFocus||pc.tactics.selected||"midjungle";state.logs.push("🆕 V1.9.9.3：18套戰術全部可指定Scrim培養；電視節目加入來賓冷卻與夜鋒明星權重；新增MSI久違回歸敘事、退役流程與第二人生身分互斥骨架。")}p.v1993Migrated=true}
function migrateProV1994(){const p=state.player;if(p.v1994Migrated)return;if(isProfessionalStage()){const pc=ensureCareer20();/* Existing saves did not store a reliable pre-change lineup snapshot, so establish the current lineup as baseline without falsely penalizing it. */pc.tactics.lineupSnapshot1994=tacticLineupSnapshot1994();enforceTacticCeilings1994("V1.9.9.4陣容上限校正");state.logs.push("🔧 V1.9.9.4：重新校正戰術陣容上限；弱點位置不再過度壓低整隊上限；熟練度不得超過上限；未來更換先發會降低受影響戰術默契。")}p.v1994Migrated=true}
migrateProV1937();migrateProV1938();migrateProV1939();migrateProV1940();migrateProV1967();migrateProV1973();migrateProV1979();migrateProV1981();migrateProV1982();migrateProV1984();migrateProV1985();migrateProV1986();migrateProV1987();migrateProV1988();migrateProV1989();migrateProV1990();migrateProV1991();migrateProV1992();migrateProV1993();migrateProV1994();msiReturnStory1993();recoverLegacyMarriageCrisis1977();
