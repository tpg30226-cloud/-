...
function finishTraining(type){
 ...
 if(type==="mental"){
  ...
  rewardCoachTrustForTraining("心理訓練",.35);state.logs.push(`心理訓練：心態 ${before.toFixed(2)} → ${p.stats.心態.toFixed(2)}，壓力 -${Math.round(stress0-p.stress)}。`);save();render();
 ...
 rewardCoachTrustForTraining(type==="review"?"復盤研究":"基礎訓練",type==="review"?.7:.55);state.logs.push(`訓練：${k} ${before.toFixed(2)} → ${p.stats[k].toFixed(2)}`);save();render();modal(`<h2>訓練完成</h2><p>${k}</p><div class="big-number">${before.toFixed(2)} → ${p.stats[k].toFixed(2)}</div><p class="goodtext">+${gain.toFixed(2)}</p>${closeBtn()}`);
}
...
function makeLocalAcquaintanceMeta(name,country,city,context,gender="女"){
 ...
 return {gender,age:18+stableAgeOffset(name,13),nationality:country,homeCountry:country,homeCity:city,currentCountry:country,currentCity:city,career,occupation:career,gameRole,role:gamer?gameRole:career,playsGame:gamer,identityType:career,publicFigure:isPublicFigureCareer(career),acquaintanceSource:`${country}・${city}${placeName(context)}`,romanceable:gender==="女",traits:["外向","好奇"],desc:`在${city}${placeName(context)}認識的${career}${gamer?`，平時也會玩${gameRole}`:""}。`};
}
...
function attemptConsensualPrivateEvent(name,kind="social"){
 ...
 if(p.condition.privateRecent>=3){
   p.condition.form=clamp((p.condition.form||70)-rand(3,7),0,100);
...
function normalizeFormerSpouseSocialState(){
 ...
 (m.divorced||[]).map(x=>x?.name).filter(Boolean).forEach(name=>finalizeFormerSpouseState(name,xReasonForFormerSpouse(name)));
}
...
function socialProfileMeta(name){
 ...
 const ex=(state.player?.romance?.marriage?.divorced||[]).some(x=>x.name===name)||c?.formerSpouse;
 return {identity:ex?"前妻":socialIdentity(name),source:story?.source||inferAcquaintanceSource(c),relationship:ex?"前妻":currentRelationshipLabel(name),special:ex?"前妻":(story?.special||c?.specialRelation||"")};
}
...
function proScrim(){
 ...
 pc.coachTrust=clamp((pc.coachTrust||50)+(win?1.2:.35),0,100);state.logs.push(`🆚 Scrim：${pc.team} ${score} ${opp}，更衣室氣氛 ${Math.round(pc.lockerRoom)}。`);
...
function marriageCard(){normalizeFormerSpouseSocialState();syncCompanionCurrentLocation();const p=state.player;if(!isProfessionalStage())return "";const spouse=p.romance?.spouse;if(spouse){const m=ensureMarriageState();return `<section class="card"><h2>💍 婚姻</h2><div class="notice">老婆：${spouse}｜婚姻信任 ${Math.round(m.trust||0)}<br>${socialLocationLabel(spouse)}</div></section>${marriageCrisisCard()}`;}const eligible=(p.romance?.partners||[]).filter(n=>(p.relations[n]||0)>=88);return eligible.length?`<section class="card"><h2>💍 婚姻</h2><p>有 ${eligible.length} 位戀愛對象符合求婚條件，由你選擇對象。</p><button id="proposeMarriage" class="reply">💍 選擇求婚對象</button></section>`:""}
...
function poachTransferApproved(){return /^同意/.test(state.player.proCareer?.transferRequest?.status||"")}
function pushPoachContractMessage(offer){
 const pc=state.player.proCareer;if(!offer||offer.contractMessageSent)return;offer.contractMessageSent=true;offer.status="談判中";
 pc.pendingExternalOffer={team:offer.team,salary:offer.salary,years:offer.years||rand(1,3),transferFee:offer.transferFee||0,type:"一軍",demands:offer.demands||[]};
 state.messages.push({id:"poach-contract-"+Date.now(),from:`${offer.team} 經紀窗口`,text:`現任俱樂部已同意進入轉會談判。${offer.transferFee?`我們願意處理約 NT$${offer.transferFee.toLocaleString()} 的轉會費。\n`:""}正式個人合約：${pc.pendingExternalOffer.years} 年，一軍，月薪 NT$${offer.salary.toLocaleString()}。`,unread:true,resolved:false,type:"poachContract",team:offer.team});
 state.logs.push(`📄 ${offer.team} 正式提出個人合約，轉會進入最後談判。`);
}
function advancePoachTransfer(){
 const p=state.player,pc=p.proCareer,o=pc.poachOffer;if(!o||o.status==="拒絕"||o.status==="失敗"||o.contractMessageSent)return;
 if(!poachTransferApproved()){o.status="等待現隊放行";return}
 const fee=Math.round(((pc.contract?.buyout||pc.contract?.salary*12||600000)*rand(65,105)/100)/1000)*1000;o.transferFee=fee;
 const buyerChance=clamp(.72+(avg()-75)*.006+(p.followers||0)/2500000-(fee>5000000?.10:0),.38,.94);
 if(Math.random()>buyerChance){o.status="失敗";state.messages.push({id:"poach-fee-fail-"+Date.now(),from:`${o.team} 經紀窗口`,text:`我們和 ${pc.team} 談過轉會費，但目前差距太大，這次交易暫時無法繼續。`,unread:true,resolved:true,type:"normal"});state.logs.push(`⛔ ${o.team} 與 ${pc.team} 未能談妥轉會費，挖角暫時中止。`);return}
 o.years=o.years||rand(1,3);pushPoachContractMessage(o);
}
function resolvePoachMessage(m,r){
 const pc=state.player.proCareer,o=pc.poachOffer;if(!o||o.team!==(m.team||String(m.from).replace(/ 經紀窗口$/, ""))){m.resolved=true;m.replied=true;save();document.querySelector('.modal-backdrop')?.remove();render();return}
 if(r==="decline"){o.status="拒絕";m.resolved=true;m.replied=true;state.logs.push(`❌ 你婉拒 ${o.team} 的挖角。`);save();document.querySelector('.modal-backdrop')?.remove();render();return}
 o.status="有興趣";m.resolved=true;m.replied=true;state.logs.push(`👀 你回覆 ${o.team}：願意進入正式轉會程序。`);
 if(poachTransferApproved())advancePoachTransfer();else{state.messages.push({id:"poach-wait-"+Date.now(),from:`${o.team} 經紀窗口`,text:`了解。我們會保留興趣；你目前仍有合約，請先取得 ${pc.team} 的轉會放行。放行後我們會直接與俱樂部談轉會費。`,unread:true,resolved:true,type:"normal"});o.status="等待現隊放行"}
 save();document.querySelector('.modal-backdrop')?.remove();render();
}
function poachDemandOptions(){return ["保證先發","補強指定位置","不過度干涉合法私生活","允許個人直播","允許個人商業代言","重大轉會先溝通"]}
function openPoachDemandTalk(m){
 const pc=state.player.proCareer,o=pc.pendingExternalOffer;if(!o)return;modal(`<h2>📝 與 ${o.team} 談附加條件</h2><p class="small">條件越多，談判破裂機率越高。</p><div class="reply-grid">${poachDemandOptions().map(x=>`<button class="reply poach-demand" data-v="${x}">${x}</button>`).join("")}</div>${closeBtn()}`);
 document.querySelectorAll('.poach-demand').forEach(b=>b.onclick=()=>{const raw=b.dataset.v;if(raw==="補強指定位置"){document.querySelector('.modal-backdrop')?.remove();chooseContractReinforcementPosition(role=>resolvePoachDemand(`補強指定位置：${contractDemandPositionLabel(role)}`,role));return}resolvePoachDemand(raw,null)});
}
function resolvePoachDemand(d,role){const pc=state.player.proCareer,o=pc.pendingExternalOffer;if(!o)return;o.demands=o.demands||[];if(o.demands.includes(d)){document.querySelector('.modal-backdrop')?.remove();render();return}const chance=clamp(.78+(avg()-80)*.008+(state.player.followers||0)/3000000-o.demands.length*.12,.30,.93);if(Math.random()<chance){o.demands.push(d);if(role)o.reinforcementDemand={role,label:contractDemandPositionLabel(role),accepted:true};state.logs.push(`📝 ${o.team} 接受轉會附加條件：「${d}」。`);state.messages.push({id:"poach-demand-ok-"+Date.now(),from:`${o.team} 經紀窗口`,text:`我們接受「${d}」。目前附加條件：${o.demands.join("、")}。你仍可回到合約訊息決定是否簽約。`,unread:true,resolved:true,type:"normal"})}else state.logs.push(`⛔ ${o.team} 拒絕附加條件：「${d}」，但目前合約報價仍有效。`);save();document.querySelector('.modal-backdrop')?.remove();render()}
function finalizeExternalPoachTransfer(){
 const p=state.player,pc=p.proCareer,o=pc.pendingExternalOffer;if(!o)return;const old=pc.team,oldRegion=fixedTeamRegion(old)||pc.region||"PCS";archiveCurrentCoaches(old);pc.team=o.team;pc.region=fixedTeamRegion(o.team)||o.region||pc.region||"PCS";pc.transferHistory=pc.transferHistory||[];pc.transferHistory.push({from:old,to:o.team,fromRegion:oldRegion,toRegion:pc.region,year:state.date.year,week:state.date.week});registerLegacyTeam(old,oldRegion);pc.stage="starter";pc.contract={team:o.team,type:"一軍",salary:o.salary,years:o.years,start:{year:state.date.year,week:state.date.week},lengthWeeks:o.years*52,demands:[...(o.demands||[])],reinforcementDemand:o.reinforcementDemand?{...o.reinforcementDemand}:null};completeContract(pc.contract);pc.roster=[];pc.coaches=[];pc.season=null;pc.transferRequest=null;pc.forcedTransferPending=null;pc.releaseRisk=Math.max(0,(pc.releaseRisk||0)-20);pc.poachOffer=null;pc.pendingExternalOffer=null;pc.subRosterInitialized=false;pc.coachTrust=50;pc.joinedAt={year:state.date.year,week:state.date.week};if(o.reinforcementDemand?.role){pc.recruitPriority=pc.recruitPriority||{};pc.recruitPriority[o.reinforcementDemand.role]=Math.max(pc.recruitPriority[o.reinforcementDemand.role]||0,3)}ensureProRoster();ensureRosterSubstitutes();initProSeason();pc.residence=currentResidenceProfile();state.logs.push(`✍️ 正式轉會：${old} → ${pc.team}，${o.years}年一軍合約，月薪 NT$${o.salary.toLocaleString()}。`);state.news.unshift(`🔄 轉會完成：夜鋒由 ${old} 加盟 ${pc.team}。`);state.messages.push({id:"transfer-done-"+Date.now(),from:`${pc.team} 管理層`,text:`轉會手續完成，歡迎加入 ${pc.team}。接下來會安排報到、隊內訓練與新賽季規劃。`,unread:true,resolved:true,type:"normal"});
}
function resolvePoachContractMessage(m,r){
 const pc=state.player.proCareer,o=pc.pendingExternalOffer;if(!o){m.resolved=true;m.replied=true;save();document.querySelector('.modal-backdrop')?.remove();render();return}
 if(r==="demands"){document.querySelector('.modal-backdrop')?.remove();openPoachDemandTalk(m);return}
 if(r==="decline"){state.logs.push(`❌ 你拒絕 ${o.team} 的正式轉會合約。`);if(pc.poachOffer)pc.poachOffer.status="拒絕";pc.pendingExternalOffer=null;m.resolved=true;m.replied=true;save();document.querySelector('.modal-backdrop')?.remove();render();return}
 m.resolved=true;m.replied=true;finalizeExternalPoachTransfer();save();document.querySelector('.modal-backdrop')?.remove();render();
}
function recoverPoachFlow(){
 const pc=state.player.proCareer,o=pc?.poachOffer;if(!isProfessionalStage()||!o?.team)return;
 const team=o.team,hasAction=(state.messages||[]).some(m=>!m.resolved&&(m.type==="poach"||m.type==="poachContract")&&(m.team===team||String(m.from||"").startsWith(team)));
 if(pc.pendingExternalOffer&&!hasAction){pushPoachContractMessage({...o,contractMessageSent:false});return}
 if((o.status==="有興趣"||o.status==="等待現隊放行"||poachTransferApproved())&&poachTransferApproved()){advancePoachTransfer();return}
 if(!hasAction&&!["拒絕","失敗"].includes(o.status)){state.messages.push({id:"poach-recover-"+Date.now(),from:`${team} 經紀窗口`,text:`先前我們向你提出過挖角，初步薪資 NT$${Number(o.salary||0).toLocaleString()}。你是否願意正式進入轉會程序？`,unread:true,resolved:false,type:"poach",team});o.status=o.status||"等待回覆"}
}
function poachingTick(){const p=state.player,pc=p.proCareer;if(!isProfessionalStage()||!pc.contract||pc.poachOffer?.status==="談判中"||Math.random()>.035)return;const teams=PRO_TEAMS.filter(x=>x!==pc.team),team=teams[rand(0,teams.length-1)];pc.poachOffer={team,salary:Math.round((pc.contract.salary*rand(105,145)/100)/1000)*1000,status:"等待回覆",createdYear:state.date.year,createdWeek:state.date.week};state.messages.push({id:"poach-"+Date.now(),from:`${team} 經紀窗口`,text:`我們對你有興趣，初步薪資可到 NT$${pc.poachOffer.salary.toLocaleString()}。若仍有合約，必須走正式轉會程序。`,unread:true,resolved:false,type:"poach",team});state.logs.push(`👀 ${team} 對夜鋒展開挖角，等待你的回覆。`)}
...
function undergroundRomanceTick(){const p=state.player;Object.values(state.characters||{}).forEach(c=>{if(!c?.publicFigure||c.relationshipType!=="地下戀人")return;c.secretRomanceRisk=clamp((c.secretRomanceRisk||5)-.5,0,100);if(c.secretRomanceRisk>=20&&Math.random()<c.secretRomanceRisk/900){c.publicRomance=true;c.relationshipType=(p.romance?.partners||[]).includes(c.name)?"戀人":"曖昧對象";ensurePublicImage().haters+=rand(2,10);state.news.unshift(`📸 媒體拍到夜鋒與${c.career||"公眾人物"} ${c.name} 私下見面，地下關係面臨曝光。`);state.logs.push(`📸 與 ${c.name} 的地下戀情曝光。`)}})}
function addSecretRomanceRisk(name,amount=5){const c=state.characters?.[name];if(c?.publicFigure&&c.relationshipType==="地下戀人")c.secretRomanceRisk=clamp((c.secretRomanceRisk||5)+amount,0,100)}
function maybeSocialInvitation(){const p=state.player;if(!isProfessionalStage()||Math.random()>.18)return;const candidates=Object.values(state.characters||{}).filter(c=>c?.known&&c.name!==p.name&&(p.relations[c.name]||0)>=35&&!c.isProStaff);if(!candidates.length)return;const c=candidates[rand(0,candidates.length-1)],adult=p.age>=18&&c.age>=18&&c.gender==="女",privateInvite=adult&&Math.random()<.18,type=privateInvite?"private":"socialInvite",label=privateInvite?"私人約會":["吃飯","看電影","喝咖啡","逛街"][rand(0,3)];state.messages.push({id:`invite-${Date.now()}`,from:c.name,text:`${privateInvite?"最近想和你單獨相處一下。":"有空嗎？想約你一起"+label+"。"}`,unread:true,resolved:false,type,inviteLabel:label});}
function ensureImageRepair(){const p=state.player;p.imageRepair=p.imageRepair||{score:0,lastScandalWeek:0};return p.imageRepair}
...
function ensureMediaLaw(){const p=state.player;p.mediaLaw=p.mediaLaw||{cases:[],leaks:[]};return p.mediaLaw}
function maybeDefamation(){if(!isProfessionalStage()||Math.random()>.025)return;const p=state.player,ml=ensureMediaLaw(),source=["匿名黑粉","八卦帳號","自媒體","前圈內人士"][rand(0,3)],truth=Math.random()<.32,claim=["私生活混亂","隊內霸凌","收錢打假賽","耍大牌"][rand(0,3)];const x={id:`law-${Date.now()}`,source,claim,truth,evidence:truth?rand(25,80):rand(5,35),status:"待處理"};ml.cases.unshift(x);state.messages.push({id:x.id,from:"經紀／法律顧問",text:`${source}公開指稱你「${claim}」。你可以澄清、蒐證或追究法律責任。`,unread:true,resolved:true,type:"legal"});}
function legalMediaCard(){if(!isProfessionalStage())return "";const ml=ensureMediaLaw(),x=ml.cases.find(x=>x.status==="待處理");return `<section class="card"><h2>⚖️ 輿論／法律</h2>${x?`<div class="notice">${x.source}：${x.claim}</div><div class="reply-grid"><button class="reply legal-action" data-a="clarify">公開澄清</button><button class="reply legal-action" data-a="sue">追究法律責任</button></div>`:`<div class="small">目前沒有待處理的惡意爆料。</div>`}<button id="leakSomeone" class="reply">🗞️ 對外爆料</button></section>`}
function handleLegalAction(a){const p=state.player,x=ensureMediaLaw().cases.find(x=>x.status==="待處理");if(!x)return;if(a==="clarify"){const good=Math.random()<clamp(.45+(100-x.evidence)*.004,.3,.9);if(good){ensurePublicImage().haters=Math.max(0,ensurePublicImage().haters-rand(5,15));ensureAudienceRating().rating=clamp(ensureAudienceRating().rating+2,0,100);x.status="已澄清"}else x.status="爭議持續";state.logs.push(`📣 對「${x.claim}」公開澄清：${good?"部分輿論接受說明":"爭議仍持續"}。`)}else{const win=!x.truth&&Math.random()<clamp(.55+(50-x.evidence)*.006,.35,.92);x.status=win?"法律處理成功":"法律處理失敗";if(win){p.cash+=rand(30000,180000);ensurePublicImage().haters=Math.max(0,ensurePublicImage().haters-15);ensureAudienceRating().rating=clamp(ensureAudienceRating().rating+4,0,100)}else if(x.truth){changeEthics(-3,"對真實爆料提告反噬")}state.logs.push(`⚖️ 法律處理：${win?"證明對方惡意造謠，獲得道歉／賠償":"未能證明對方惡意造謠"}。`)}save();render()}
...
function ensureRosterSubstitutes(){const p=state.player,pc=p.proCareer;if(!isProfessionalStage()||!pc.roster)return;let subs=pc.roster.filter(x=>x.isSub);if(pc.subRosterInitialized)return;pc.subRosterInitialized=true;const count=stableAgeOffset(pc.team+state.date.year,3);const roles=["上路","打野","中路","ADC","輔助"];for(let i=0;i<count;i++){const role=roles[stableAgeOffset(pc.team+"sub"+i,5)],name=`${pc.team.split(" ")[0]}·${["Echo","Mori","Kai","Jun","Neo"][stableAgeOffset(pc.team+i,5)]}${i+1}`;pc.roster.push({name,role,isPlayer:false,isSub:true,rating:rand(62,82),relation:50,trust:50,chemistry:45});addSocialAcquaintance(name,50,{gender:"男",age:18+stableAgeOffset(name,8),role,isPro:true,identityType:"替補選手",team:pc.team,acquaintanceSource:`${pc.team} 戰隊`})}}
function scoutingTalentPool(){const p=state.player,db=ensureGlobalProDatabase(),out=[];Object.values(state.characters||{}).filter(c=>c.playsGame&&!c.isPro).slice(0,5).forEach(c=>out.push({name:c.name,role:c.role||diversifiedGameRole(c.name),source:"高分路人",rating:60+stableAgeOffset(c.name,28)}));["校際賽冠軍MVP","杯賽冠軍","菁英榜路人王","次級聯賽新秀"].forEach((source,i)=>out.push({name:["夜鴉","白河","Kirin","Rook"][i],role:["打野","ADC","中路","上路"][i],source,rating:68+stableAgeOffset(source+state.date.year,24)}));for(const r of PRO_REGIONS){const teams=Object.values(db[r]||{});if(teams[0]?.[0])out.push({...teams[0][0],source:"現役職業選手"})}return out.slice(0,12)}
function openRecruitSuggestionV1918(){const cands=scoutingTalentPool();modal(`<h2>🧲 球探／挖角人才池</h2><div class="reply-grid">${cands.map((c,i)=>`<button class="reply recruit1918" data-i="${i}">${c.name}｜${c.role}｜${c.source}｜評價 ${c.rating}</button>`).join("")}</div>${closeBtn()}`);document.querySelectorAll('.recruit1918').forEach(b=>b.onclick=()=>{const c=cands[+b.dataset.i];state.logs.push(`🧲 你建議戰隊觀察 ${c.name}（${c.source}／${c.role}）。管理層將評估試訓、合約與轉會費。`);save();document.querySelector('.modal-backdrop')?.remove();render()})}
...
function commercialCard(){if(!isProfessionalStage())return "";const a=ensureCommercial();return `<section class="card"><h2>💼 商業合約</h2><div class="small">同類別排他，不同類別可同時簽約。直播平台另有每月履約條件。</div>${a.map(x=>`<div class="notice">${x.category}｜${x.brand}｜每4週 NT$${x.pay.toLocaleString()}${x.category==="直播平台"?`｜直播 ${x.done||0}/${x.required} 次`:""}</div>`).join("")}<button id="seekCommercial" class="reply">尋找商業合作</button></section>`}
function seekCommercial(){const a=ensureCommercial(),free=COMMERCIAL_CATEGORIES.filter(c=>!a.some(x=>x.category===c));if(!free.length)return;const cat=free[rand(0,free.length-1)],brand=`${["Apex","Nova","Pulse","Vertex","Prime"][rand(0,4)]} ${cat}`,pay=Math.round((rand(30000,180000)+(state.player.followers||0)/20)/1000)*1000,required=cat==="直播平台"?rand(6,12):0;a.push({category:cat,brand,pay,weeks:rand(12,52),lastPay:state.date.week,required,done:0});state.logs.push(`💼 簽下 ${brand}「${cat}」合作。${required?`每4週需直播至少 ${required} 次。`:""}`);save();render()}
...
function buildInternationalTournament(kind){
 ...
 if(kind==="MSI"){const e=[];PRO_REGIONS.forEach(r=>e.push(r===myRegion?team:`${r} Spring #1`,`${r} Spring #2`));const u=[...new Set(e)].slice(0,10);ev.groups=[{name:"A",teams:u.filter((_,i)=>i%2===0),standings:[]},{name:"B",teams:u.filter((_,i)=>i%2===1),standings:[]}];ev.format="雙循環；各組前二晉級四強BO5";ev.stage="分組賽";}
   ...
   const e=[],bonusRegion=msiChampionRegionForYear(state.date.year)||myRegion;
   PRO_REGIONS.forEach(r=>e.push(r===myRegion?team:`${r} Summer #1`,`${r} Summer #2`,`${r} Summer #3`));
   e.push(`${bonusRegion} Summer #4`);
   const u=[...new Set(e)];while(u.length<16)u.push(`Wildcard ${u.length+1}`);
   ev.groups=["A","B","C","D"].map((g,i)=>({name:g,teams:u.filter((_,j)=>j%4===i).slice(0,4),standings:[]}));
   ev.msiBonusRegion=bonusRegion;ev.format=`四組雙循環；各組前二晉級八強BO5。${bonusRegion} 因奪得MSI冠軍獲得第4種子席位。`;ev.stage="抽籤完成";
 }
...
function internationalQualificationRules(){
 ...
 WORLDS:"五大賽區夏季前三各3席，共15隊；MSI冠軍所屬賽區額外獲得1張世界賽門票，因此該賽區夏季第4種子也能晉級世界賽。固定16隊，四組各4隊雙循環，各組前二進BO5八強。"};
}
...
function runRichLeagueMatch(){const difficultyPressure=.025+Math.max(0,(state.player.proCareer?.careerStats?.seriesW||0)-(state.player.proCareer?.careerStats?.seriesL||0))*.002+ruptureMatchPenalty();
 ...
 pc.lastMatch={team:pc.team,opp:opp.name,win:my>his,score:`${my}:${his}`,logs:[...logs],k,d,a,cs,mvp,year:state.date.year,week:state.date.week,day:state.date.day,phase:sn.phase||"正式比賽",postInterviewDone:false};
 pc.pendingPostInterview=true;
...
function transferMarketCard(){
 ...
 if(!active&&!list.length&&!hist.length)return "";
 return `<section class="card"><h2>🏷️ 轉會掛牌市場</h2><div class="small">${active?"目前是轉會期。戰隊可因成績、財務、戰術適配、紀律或陣容重整掛牌選手；掛牌後不保證有人購買。":"目前非轉會期，顯示最近掛牌紀錄。"} 成交去向可能是一軍，也可能是次級聯賽／二隊。</div>
 ${list.length?`<div class="log">${list.map(x=>`<strong>${x.name}</strong>｜${x.team}｜${x.role}｜評價 ${x.rating}<br>原因：${x.reason}｜要價 NT$${Number(x.asking||0).toLocaleString()}｜狀態：${x.status}${x.buyer?` → ${x.buyer}（${x.destination}）`:""}`).join("<br><br>")}</div>`:`<div class="small">本轉會窗目前沒有掛牌選手。</div>`}
 ${hist.length?`<div class="notice"><strong>最近成交／未售出</strong><br>${hist.map(x=>`${x.name}｜${x.status}${x.buyer?`｜${x.buyer}・${x.destination}`:"｜留隊"}`).join("<br>")}</div>`:""}</section>`;
}
...
function locationRestrictedSocialAction(act){
 return ["food","cafe","movie","date","intimate","private","chat","hangout","latefood","arcade"].includes(act);
}
