...
function ensureCareer20(){const p=state.player,pc=p.proCareer||{};pc.tactics=pc.tactics||{selected:"midjungle",mastery:{midjungle:62,botcarry:55,topside:52,split:45,teamfight:60,tempo:56,scaling:58,junglecarry:48,midcarry:64,doublecarry:58},history:[]};pc.tactics.mastery=pc.tactics.mastery||{};Object.keys(TACTIC_DEFS).forEach(k=>{if(!Number.isFinite(pc.tactics.mastery[k]))pc.tactics.mastery[k]=45});pc.clubEquity=pc.clubEquity||{team:pc.team||null,shares:{},dividends:0};pc.clubEquity.shares=pc.clubEquity.shares||{};pc.legacy=pc.legacy||{starPower:0,rivals:{},enemies:{},apprentices:{},hallOfFame:{eligible:false,inducted:false,consideredYears:[]},postCareer:null};pc.mediaNetwork=pc.mediaNetwork||[];ensureCoachProfile();return pc}
...
function tacticsCard(){if(!isProfessionalStage())return "";const pc=ensureCareer20(),coach=ensureCoachProfile(),sel=pc.tactics.selected||"midjungle",fit=tacticFit(sel),ceil=tacticAbilityCeiling(sel),c=coach?.skills||{};return `<section class="card"><div class="row space"><h2>🧠 戰隊戰術體系</h2><span class="badge">教練安排：${TACTIC_DEFS[sel].name}</span></div><div class="notice">${TACTIC_DEFS[sel].desc}<br>目前適配：${fit>=4?"非常適合":fit>=1?"適合":fit>-2?"普通":"不理想"}｜熟練度 ${Math.round(pc.tactics.mastery[sel])}／陣容上限約 ${ceil}</div>${coach?`<div class="small">主教練 ${coach.name}｜BP ${c.bp}｜戰術 ${c.tactics}｜版本 ${c.meta}｜臨場 ${c.adjust}｜管理 ${c.management}<br>擅長：${coach.preferredTactics.map(k=>TACTIC_DEFS[k].name).join("、")}</div>`:""}<div class="reply-grid"><button class="reply tactic-practice" data-tactic="${sel}">🏋️ 練習目前體系</button><button class="reply tactic-suggest" data-tactic="midjungle">💬 建議重視中野</button><button class="reply tactic-suggest" data-tactic="botcarry">💬 建議重視下路</button></div><div class="small">正式比賽戰術只能由教練決定。選手能練習、提出意見，但不能自行指定比賽打法；選手能力與英雄池會限制戰術天花板。</div></section>`}
...
function clubEquityCard(){if(!isProfessionalStage())return "";const p=state.player,pc=ensureCareer20(),pct=equityPct(),price=equityPricePerPct(),md=pc.managementDirective;return `<section class="card"><div class="row space"><h2>🏢 ${pc.team} 股權</h2><span class="badge">持股 ${pct.toFixed(1)}%</span></div><div class="notice">目前權力：${equityRights(pct)}<br>估計每1%股權：NT$${price.toLocaleString()}${md?`<br>最近管理事項：${md.type}${md.role?`・${md.role}`:""}｜${md.status}`:""}</div><div class="reply-grid"><button class="reply equity-buy" data-pct="1">購買1%</button><button class="reply equity-buy" data-pct="5">購買5%</button>${pct>=20?`<button class="reply equity-direct" data-role="ADC">指示補強下路</button><button class="reply equity-direct" data-role="上路">指示補強上路</button><button class="reply equity-direct" data-role="打野">指示補強打野</button><button class="reply coach-change">🧑‍🏫 提案更換總教練</button>`:`<button class="reply coach-suggest">💬 建議評估總教練</button>`}</div><div class="small">正式比賽戰術由教練負責。若教練長期判斷不佳，低持股可要求管理層回覆評估；達20%後可正式推動總教練更換。</div></section>`}
...
function startMajorInterview(ctx,after){const pc=ensureCareer20(),host=ensureMediaHost(),opp=currentScheduledProMatch()?.opp||currentProOpponent()?.name||"下一個對手",questions=[{q:`${host.name}：「${ctx.title}即將開始。面對 ${opp}，你認為現在最重要的是什麼？」`,a:[["team","把團隊準備做好"],["confident","我們的目標就是贏"],["calm","先專注每一個細節"]]},{q:`${host.name}：「外界對夜鋒與 ${pc.team} 的期待很高，你如何面對這種關注？」`,a:[["team","壓力由全隊一起承擔"],["self","我會用表現回應"],["calm","不讓外界改變準備節奏"]]},{q:`${host.name}：「如果這次結果不如預期，你最希望外界記住什麼？」`,a:[["team","我們會一起承擔結果"],["confident","我們還沒想過輸"],["self","我會先檢討自己"]]}];pc.activeInterview={ctx,host,questions,index:0,answers:[],after};showInterviewQuestion()}
...
function proScrim(){
 ...
 const tk=pc.tactics?.selected||"midjungle",ceil=tacticAbilityCeiling(tk),cur=pc.tactics.mastery[tk]||45,room=Math.max(0,ceil-cur),tg=room>0?Math.min(room,(win?.8:.35)*Math.max(.25,room/18)):0;pc.tactics.mastery[tk]=clamp(cur+tg,0,100);pc.coachTrust=clamp((pc.coachTrust||50)+(win?1.2:.35),0,100);state.logs.push(`🆚 Scrim：${pc.team} ${score} ${opp}，更衣室氣氛 ${Math.round(pc.lockerRoom)}。${tg>0?` ${TACTIC_DEFS[tk].name}熟練 +${tg.toFixed(1)}。`:" 戰術已接近目前陣容上限。"}`);
