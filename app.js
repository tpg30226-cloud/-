...
function career(){
 ...
 <section class="card"><h2>版本</h2><div class="log"><strong>V1.9.8.5</strong>｜真實職業世界：個性與狀態正式影響比賽、隊伍化學反應、對手研究／情報、職涯需求與成長曲線、故事型師徒／宿敵、完整開季專訪、全隊自動周邊、小孩性別與生涯數據中心。</div></section>`;
}
function bind(){
...
function contextualInterviewQuestions1984(ctx,host,opp){const pc=state.player.proCareer,w=ensureLivingWorld1984(),phase=proAnnualPhase(),bo5=/季後賽|MSI|世界賽/.test(ctx.title+phase),rival=pc.rivals?.[opp],qs=[];if(bo5)qs.push({q:`${host.name}：「如果要你預測這個BO5的比分，你會選哪一個？」`,a:[["3-0","3：0"],["3-1","3：1"],["3-2","3：2"],["no-score","不預測比分"]]});qs.push({q:`${host.name}：「目前是${RHYTHM_META_1984[w.rhythm.key]?.name||"新版本"}，你覺得 ${pc.team} 的適應如何？」`,a:[["meta-ready","我們準備得很好"],["meta-adjust","還在調整，但會找到答案"],["meta-team","版本只是條件，執行更重要"]]});if(rival)qs.push({q:`${host.name}：「你和 ${opp} 已經多次交手，這場對你來說是不是宿敵戰？」`,a:[["rival-respect","我尊重這個對手"],["rival-fire","我就是想再贏他一次"],["rival-calm","歷史不會替今天打比賽"]]});qs.push({q:`${host.name}：「外界認為冠軍隊已經被大量研究，你們會不會擔心原本的招牌戰術失效？」`,a:[["change","我們準備了新的東西"],["trust","被研究也不代表能阻止我們"],["team","臨場調整才是關鍵"]]});return qs.slice(0,5)}
...
migrateProV1937();migrateProV1938();migrateProV1939();migrateProV1940();migrateProV1967();migrateProV1973();migrateProV1979();migrateProV1981();migrateProV1982();migrateProV1984();migrateProV1985();recoverLegacyMarriageCrisis1977();
