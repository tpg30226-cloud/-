...
function clubEquityCard(){if(!isProfessionalStage())return "";const p=state.player,pc=ensureCareer20(),pct=equityPct(),price=equityPricePerPct(),md=pc.managementDirective,cf=pc.clubFinance,tasks=(pc.managementTasks||[]).slice(-3).reverse();return `<section class="card"><div class="row space"><h2>🏢 ${pc.team} 股權／經營</h2><span class="badge">持股 ${pct.toFixed(1)}%</span></div><div class="notice">目前權力：${equityRights(pct)}｜管理層關係 ${Math.round(pc.managementRelation)}/100<br>估計每1%股權：NT$${price.toLocaleString()}<br>俱樂部現金 NT$${Math.round(cf.cash).toLocaleString()}｜本年營收 NT$${Math.round(cf.revenue).toLocaleString()}｜支出 NT$${Math.round(cf.expenses).toLocaleString()}${md?`<br>最近管理事項：${md.type}${md.role?`・${md.role}`:""}｜${md.status}`:""}</div><div class="reply-grid"><button class="reply equity-buy" data-pct="1">購買1%</button><button class="reply equity-buy" data-pct="5">購買5%</button>${pct>=20?`<button class="reply equity-direct" data-role="上路">補強上路</button><button class="reply equity-direct" data-role="打野">補強打野</button><button class="reply equity-direct" data-role="中路">補強中路</button><button class="reply equity-direct" data-role="ADC">補強ADC</button><button class="reply equity-direct" data-role="輔助">補強輔助</button><button class="reply equity-direct" data-role="替補">補充替補</button><button class="reply coach-change">🧑‍🏫 提案更換總教練</button><button id="capitalInjection" class="reply">💰 股東增資</button>`:`<button class="reply coach-suggest">💬 建議評估總教練</button>`}</div>${tasks.length?`<div class="small">${tasks.map(x=>`📋 ${x.type}${x.role?`・${x.role}`:""}｜${x.status}${x.note?`｜${x.note}`:""}`).join("<br>")}</div>`:""}<div class="small">正式指示會形成管理任務並逐週回報；若管理層長期無視正式指示，雙方關係會惡化。</div></section>`}
...
function clubFinanceWeeklyTick(){if(!isProfessionalStage())return;const p=state.player,pc=ensureCareer20(),f=pc.clubFinance,star=pc.legacy?.starPower||Math.min(100,Math.round((p.followers||0)/12000)),base=1200000+star*35000+Math.min(2500000,(p.followers||0)*2);f.revenue+=base;f.sponsorRevenue+=Math.round(base*.45);f.merchRevenue+=Math.round(base*.20);f.eventRevenue+=Math.round(base*.12);f.cash+=base;const weeklyCost=Math.round(clubPayrollEstimate()/52+650000);f.expenses+=weeklyCost;f.cash-=weeklyCost;if(state.date.week%8===0)state.logs.push(`🏢 ${pc.team} 財務：本年營收 NT$${Math.round(f.revenue).toLocaleString()}｜支出 NT$${Math.round(f.expenses).toLocaleString()}。`);if(f.lastYear!==state.date.year){const profit=f.revenue-f.expenses,rate=profit>0?f.dividendRate:0,pool=Math.max(0,Math.round(profit*rate)),mine=Math.round(pool*equityPct()/100);if(mine>0&&f.lastDividendYear!==f.lastYear){p.cash+=mine;pc.clubEquity.dividends=(pc.clubEquity.dividends||0)+mine;state.logs.push(`💰 ${pc.team} 股東分紅：夜鋒持股 ${equityPct().toFixed(1)}%，收到 NT$${mine.toLocaleString()}。`)}f.lastDividendYear=f.lastYear;f.lastYear=state.date.year;f.revenue=0;f.expenses=0;f.merchRevenue=0;f.eventRevenue=0;f.sponsorRevenue=0;f.prizeRevenue=0}}
function managementTaskTick(){if(!isProfessionalStage())return;const pc=ensureCareer20(),tasks=pc.managementTasks||[];for(const t of tasks.filter(x=>!["完成","失敗","取消"].includes(x.status))){t.age=(t.age||0)+1;if(t.age===1){t.status="執行中";t.note=t.type==="補強"?"球探已提出候選名單":"管理層正在評估"}else if(t.age>=2){const success=Math.random()<.62;if(success){t.status="完成";t.note=t.type==="補強"?`已鎖定${t.role}補強方案，進入合約／註冊程序`:"評估完成並正式回報";pc.managementRelation=clamp(pc.managementRelation+2,0,100)}else{t.status="失敗";t.note=["球員本人拒絕加盟","對方戰隊拒絕出售","薪資／轉會預算不足","目前沒有符合要求的人選"][rand(0,3)];pc.managementRelation=clamp(pc.managementRelation-2,0,100)}pc.managementDirective=t;state.logs.push(`🏢 管理層回報：${t.type}${t.role?`「${t.role}」`:""}${t.status}｜${t.note}`)}}}
