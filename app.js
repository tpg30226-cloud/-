...
function career(){
 ...
 <section class="card"><h2>版本</h2><div class="log"><strong>V1.9.6.7</strong>｜動態世界・人物深化篇：前妻復合再婚、低頻個性化出軌、跨國聊天、NPC主動生活、人物成長、校園／師徒、媒體與法律PR、職業心理健康、高端資產與多人派對旅行。</div></section>`;
}
function bind(){
...
function privatePartyCard(){if(!isProfessionalStage())return "";const p=ensureLifestyle(),owned=[...p.assets.vehicles,...p.assets.homes],hasYacht=owned.includes("yacht")||owned.includes("superyacht"),hasVilla=owned.includes("villa");if(!hasYacht&&!hasVilla)return "";return `<section class="card"><h2>🎉 私人活動</h2><div class="small">豪宅／遊艇可舉辦生日、一般聚會與成年人限定私人派對。成人私人派對曝光率低，但若外流可能造成較大的形象、贊助與人際後果。</div><div class="reply-grid"><button id="normalPrivateParty" class="reply">🎂 舉辦私人派對</button><button id="adultPrivateParty" class="reply" ${state.player.age<18?"disabled":""}>🔞 成人私人派對</button></div></section>`}
function runPrivateParty(adult=false){const p=state.player;if(adult&&p.age<18)return;const adults=Object.values(state.characters||{}).filter(c=>c?.known&&Number(c.age||0)>=18&&proSocialAllowed(c)).sort((a,b)=>(p.relations[b.name]||0)-(p.relations[a.name]||0)).slice(0,8);if(!consume(adult?"成人私人派對":"私人派對",1))return;const guests=adults.filter(c=>Math.random()<clamp(.25+(p.relations[c.name]||0)/160,.25,.82)).slice(0,6);guests.forEach(c=>p.relations[c.name]=clamp((p.relations[c.name]||0)+rand(1,3),0,100));const exposure=adult&&Math.random()<clamp(.008+guests.length*.004+(guests.some(c=>c.publicFigure)?.012:0),.008,.06);if(exposure){changeCareerRep(-rand(6,14),"私人成人派對曝光");p.prCrisis={type:"私人成人派對照片外流",severity:rand(3,5),source:"匿名外流"};state.news.unshift("🚨 夜鋒高度私密的成人派對內容外流，引發贊助商與媒體關注。")};state.logs.push(`🎉 ${adult?"成人私人":"私人"}派對完成，來賓 ${guests.map(x=>x.name).join("、")||"少數熟人"}${exposure?"；事件意外曝光":"；維持私人"}。`);save();render();modal(`<h2>🎉 派對結束</h2><p>參加者：${guests.map(x=>x.name).join("、")||"少數熟人"}</p>${exposure?`<div class="notice badtext">⚠️ 低機率曝光事件發生，造成較大的公關風險。</div>`:`<div class="notice goodtext">活動維持私人，沒有影響職業風評。</div>`}${closeBtn()}`)}
